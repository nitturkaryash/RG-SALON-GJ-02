...
if (!fs.existsSync(traceMappingPath)) {
  ...
  if (fs.existsSync(path.join(process.cwd(), 'mock-trace-mapping.cjs'))) {
    const mockContent = fs.readFileSync(path.join(process.cwd(), 'mock-trace-mapping.cjs'), 'utf8');
    fs.writeFileSync(path.join(traceMappingPath, 'dist/trace-mapping.umd.js'), mockContent);
    console.log('Copied mock-trace-mapping.cjs to trace-mapping module');
  } else {
...
try {
  execSync('node verify-modules.cjs', { stdio: 'inherit' });
} catch (error) {
...
try {
  if (fs.existsSync(path.join(process.cwd(), 'babel-generator-patch.cjs'))) {
    execSync('node babel-generator-patch.cjs', { stdio: 'inherit' });
  } else {
    console.log('babel-generator-patch.cjs not found, skipping patch');
  }
} catch (error) {
