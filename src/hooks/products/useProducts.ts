...
): Promise<{ success: boolean; updatedProducts?: any[]; error?: any }> => {
  try {
    console.log(
      '🔸 INVENTORY UPDATE: Starting inventory update for products:',
      updates
    );
    ...
    for (const update of updates) {
      console.log(
        `🔸 INVENTORY UPDATE: Validating product ID ${update.productId} for inventory update`
      );
      ...
      if (!isValidUuid) {
        console.warn(
          `Invalid UUID format for product ID: ${update.productId}. Skipping update.`
        );
    ...
    console.log(
      `🔸 INVENTORY UPDATE: Processing ${validUpdates.length} valid product updates`
    );
    ...
    for (const update of validUpdates) {
      try {
        console.log(
          `🔸 INVENTORY UPDATE: Updating stock for product ID ${update.productId} by ${update.quantity}`
        );
        ...
        if (fetchError) {
          console.error(
            `Error fetching product with ID ${update.productId}:`,
            fetchError
          );
          errors.push(
            `Error fetching product with ID ${update.productId}: ${fetchError.message}`
          );
          allUpdatesSuccessful = false;
          continue;
        }
        ...
        console.log(
          `🔸 INVENTORY UPDATE: Current stock for ${product.name} (${update.productId}): ${currentStock}`
        );
        ...
        if (rpcError) {
          console.error(
            `Error updating stock via RPC for product ${update.productId}:`,
            rpcError
          );
          errors.push(
            `Error updating stock via RPC for product ${update.productId}: ${rpcError.message}`
          );
          allUpdatesSuccessful = false;
        } else {
          console.log(
            `🔸 INVENTORY UPDATE: Successfully decremented stock for ${product.name} by ${update.quantity}`
          );
          ...
          if (verifyError) {
            console.error(
              `Error verifying stock update for product ${update.productId}:`,
              verifyError
            );
          } else {
            const verifiedStock = verifyData?.stock_quantity ?? 0;
            console.log(
              `🔸 INVENTORY UPDATE: Verified stock for ${product.name}: ${verifiedStock}`
            );
        ...
        console.error(
          `Error processing product update for ID ${update.productId}:`,
          itemError
        );
        errors.push(
          `Error processing product update for ID ${update.productId}: ${itemError instanceof Error ? itemError.message : 'Unknown error'}`
        );
        allUpdatesSuccessful = false;
      }
    }
    ...
    return {
      ...
      error:
        errors.length > 0
          ? { message: 'Some updates failed', details: errors }
          : undefined,
    };
  } catch (error) {
  ...
  const fetchProducts = async () => {
    ...
    try {
      ...
      const { data, error: fetchError } = await supabase
        .from('product_master')
        .select(
          'id, name, price, hsn_code, units, product_type, mrp_incl_gst, mrp_excl_gst, gst_percentage, stock_quantity, active, created_at, updated_at'
  ...
  const addProduct = async (
    ...
    try {
      ...
      const { data: existing, error: checkError } = await supabase
        ...
        .or(
          `name.ilike.${product.product_name},hsn_code.eq.${product.hsn_code || ''}`
        );
      ...
      if (existing && existing.length > 0) {
        ...
        if (nameClash) {
          throw new Error(
            `A product named "${product.product_name}" already exists`
          );
        }
        const hsnClash = existing.find(
          p => (p.hsn_code || '') === (product.hsn_code || '')
        );
        if (hsnClash && product.hsn_code) {
          throw new Error(
            `HSN "${product.hsn_code}" already exists for ${hsnClash.name}`
          );
        }
      }
