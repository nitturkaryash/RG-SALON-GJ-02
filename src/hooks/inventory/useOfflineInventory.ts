    ...
    mutationFn: async ({
      ...
      if (isOnline) {
        try {
          ...
          console.error(
            '❌ Failed to update inventory online, storing offline:',
            error
          );
          ...
          toast.warn(
            '⚠️ Inventory updated offline - will sync when connection is restored'
          );
        }
      } else {
        await offlineStorage.storeOffline(
    ...
    mutationFn: async (
      ...
      if (isOnline) {
        try {
          ...
          console.error(
            '❌ Failed to save inventory online, storing offline:',
            error
          );
          ...
          toast.warn(
            '⚠️ Inventory item saved offline - will sync when connection is restored'
          );
        }
      } else {
    ...
    mutationFn: async (itemId: string): Promise<void> => {
      if (isOnline) {
        try {
          ...
          toast.warn(
            '⚠️ Inventory item marked for deletion - will sync when connection is restored'
          );
        }
      } else {
        await offlineStorage.storeOffline(
        ...
        toast.info(
          '📱 Inventory item marked for deletion - will sync when online'
        );
      }
    },
    onSuccess: (_, itemId) => {
