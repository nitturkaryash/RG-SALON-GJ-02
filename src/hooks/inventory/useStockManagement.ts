...
export function useStockManagement() {
      ...
      reader.onload = e => {
        try {
          ...
          if (!workbook.SheetNames.includes('STOCK DETAILS')) {
            reject(
              new Error('Excel file must contain a sheet named "STOCK DETAILS"')
            );
            return;
          }
          ...
          for (let i = 0; i < jsonData.length; i++) {
            ...
            if (row['Product Name'] === 'PURCHASE - STOCK IN') {
              ...
              row['Product Name'] === 'SALES TO CUSTOMER - STOCK OUT'
            ) {
              currentSection = 'sales';
              continue;
            } else if (
              row['Product Name'] === 'SALON CONSUMPTION - STOCK OUT'
            ) {
  ...
  const extractStockData = async (file: File) => {
    ...
    try {
      ...
      for (const product of data.uniqueProducts) {
        ...
        if (queryError)
          throw new Error(
            `Error checking for existing product: ${queryError.message}`
          );
      ...
      if (data.purchases.length > 0) {
        ...
        if (purchasesError)
          throw new Error(
            `Error inserting purchases: ${purchasesError.message}`
          );
      }
      ...
      if (data.consumption.length > 0) {
        ...
        if (consumptionError)
          throw new Error(
            `Error inserting consumption: ${consumptionError.message}`
          );
      }
      ...
      for (const balance of data.balance) {
        ...
        if (queryError)
          throw new Error(
            `Error checking for existing balance: ${queryError.message}`
          );
        ...
        if (existingBalance && existingBalance.length > 0) {
          ...
          if (insertError)
            throw new Error(`Error inserting balance: ${insertError.message}`);
        }
      }
  ...
  const fetchCashSales = async () => {
    ...
    try {
      ...
      if (salesError)
        throw new Error(`Error fetching cash sales: ${salesError.message}`);
  ...
  const convertTransactions = async (transactionIds: string[]) => {
    ...
    try {
      ...
      for (const saleId of transactionIds) {
        ...
        if (saleError)
          throw new Error(`Error fetching sale: ${saleError.message}`);
        ...
        if (consumptionError)
          throw new Error(
            `Error creating consumption: ${consumptionError.message}`
          );
        ...
        if (updateError)
          throw new Error(`Error updating sale: ${updateError.message}`);
  ...
  const fetchInsights = async () => {
    ...
    try {
      ...
      const purchaseProductIds = recentPurchases.map(
        purchase => purchase.product_id
      );
      const saleProductIds = recentSales.map(sale => sale.product_id);
      const allProductIds = [
        ...new Set([...purchaseProductIds, ...saleProductIds]),
      ];
      ...
      const insightData = {
        ...
        totalInventoryValue: formattedPurchases.reduce(
          (sum, item) => sum + item.invoice_value,
          0
        ),
        averagePurchaseValue:
          formattedPurchases.length > 0
            ? formattedPurchases.reduce(
                (sum, item) => sum + item.invoice_value,
                0
  ...
  const processStats = () => {
    ...
    try {
      ...
      const categoryDistribution = [
        ...insights.recentPurchases,
        ...insights.recentSales,
      ]
        .reduce((acc: any[], item: any) => {
