...
export const usePurchaseHistory = () => {
  ...
  const fetchPurchases = useCallback(async () => {
    ...
    try {
      ...
      const transformedData: PurchaseTransaction[] = (
        purchaseRecords as any[]
      ).map(item => {
        ...
        return {
          ...
          purchase_cost_per_unit_ex_gst: Number(
            item['Purchase_Cost/Unit(Ex.GST)'] || 0
          ),
          discount_on_purchase_percentage: Number(
            item.discount_on_purchase_percentage || 0
          ),
          ...
          purchase_invoice_value_rs: Number(
            item.purchase_invoice_value_rs || 0
          ),
  ...
  const deletePurchaseTransaction = useCallback(
    async (purchaseId: string): Promise<boolean> => {
      try {
        ...
        console.log(
          'Purchase transaction deleted successfully with enhanced tracking'
        );
  ...
  const updatePurchaseTransaction = useCallback(
    async (purchaseId: string, updateData: UpdatePurchaseTransactionData) => {
      try {
        ...
        const purchaseFormData: PurchaseFormData = {
          ...
          product_name:
            updateData.product_name || existingPurchase.product_name,
          vendor:
            updateData.supplier ||
            updateData.Vendor ||
            ...
            '',
          invoice_number:
            updateData.purchase_invoice_number ||
            existingPurchase.purchase_invoice_number ||
            '',
          hsn_code: updateData.hsn_code || existingPurchase.hsn_code || '',
          unit_type: updateData.units || existingPurchase.units || '',
          purchase_qty:
            updateData.purchase_qty ?? existingPurchase.purchase_qty,
          gst_percentage:
            updateData.gst_percentage ?? existingPurchase.gst_percentage ?? 18,
          mrp_incl_gst:
            updateData.mrp_incl_gst ?? existingPurchase.mrp_incl_gst ?? 0,
          mrp_excl_gst:
            updateData.mrp_excl_gst ?? existingPurchase.mrp_excl_gst ?? 0,
          mrp_per_unit_excl_gst:
            updateData.mrp_excl_gst ?? existingPurchase.mrp_excl_gst ?? 0,
          discount_on_purchase_percentage:
            updateData.discount_on_purchase_percentage ??
            existingPurchase.discount_on_purchase_percentage ??
            0,
          purchase_cost_taxable_value:
            updateData.purchase_taxable_value ??
            existingPurchase.purchase_taxable_value ??
            0,
          purchase_invoice_value:
            updateData.purchase_invoice_value_rs ??
            existingPurchase.purchase_invoice_value_rs ??
            0,
          is_interstate:
            (updateData.purchase_igst ?? existingPurchase.purchase_igst ?? 0) >
            0,
        };
        ...
        console.log(
          'Purchase transaction updated successfully with enhanced tracking'
        );
  ...
  const addPurchaseTransactionEnhanced = useCallback(
    async (purchaseData: PurchaseFormData) => {
      try {
        ...
        if (!result.success) {
          ...
          return {
            ...
            error: {
              message:
                result.error?.message || 'Failed to add purchase transaction',
            },
          };
        }

        console.log(
          'Purchase transaction added successfully with enhanced tracking'
        );
        ...
        const errorMessage =
          err instanceof Error
            ? err.message
            : 'Failed to add purchase transaction';
        setError(errorMessage);
        return {
          success: false,
          error: { message: errorMessage },
  ...
  const addOpeningBalance = useCallback(
    async (
      openingData: OpeningBalanceData
      ...
      try {
        ...
        console.log(
          `Current stock: ${currentStock}, Opening balance: ${openingData.opening_qty}, Total after opening: ${totalStockAfterOpening}`
        );
        ...
        const openingBalancePayload = {
          ...
          discount_on_purchase_percentage:
            openingData.discount_on_purchase_percentage,
          ...
          'Purchase_Cost/Unit(Ex.GST)':
            openingData.purchase_cost_per_unit_ex_gst,
        ...
        console.log(
          'Opening balance payload for purchase_history_with_stock:',
        ...
        if (insertError) {
          console.error(
            'Error inserting opening balance into purchase_history_with_stock:',
            insertError
          );
        ...
        console.log(
          'Opening balance inserted into purchase_history_with_stock:',
          insertedData
        );
        ...
        if (currentProduct) {
          ...
          if (updateError) {
            ...
            console.log(
              `Product stock updated: ${currentStock} -> ${totalStockAfterOpening}`
            );
          }
        } else {
          console.warn(
            'Product not found in products table, stock_quantity not updated (assumed new product).'
          );
        }
        ...
        return {
          ...
          error:
            err instanceof Error
              ? err
              : new Error('Failed to add opening balance'),
        };
      }
    },
    []
