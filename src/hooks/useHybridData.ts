...
import { hybridDataService, DataOperation, SyncDirection } from '../utils/hybridDataService';
...
interface HybridDataResult<T = any> {
  ...
  // Operations
  create: (data: Partial<T>) => Promise<{ success: boolean; source: string; id?: string }>;
  update: (id: string, data: Partial<T>) => Promise<{ success: boolean; source: string }>;
...
export function useHybridData<T = any>(options: UseHybridDataOptions): HybridDataResult<T> {
  const { table, autoSync = true, syncInterval = 30000, conflictResolution = 'latest-wins' } = options;
  
  const [data, setData] = useState<T[]>([]);
  const [isLoading, setIsLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [source, setSource] = useState<'cache' | 'local' | 'remote' | 'hybrid'>('cache');
  ...
  const loadData = useCallback(async (showLoading = true) => {
    ...
    try {
      ...
      console.log(`📊 Loaded ${result.data.length} records from ${result.source} for table: ${table}`);
      ...
      try {
        ...
        if (fallbackResult.data.length > 0) {
          ...
          console.log(`🔄 Fell back to local data: ${fallbackResult.data.length} records`);
        }
      } catch (fallbackErr) {
  ...
  const create = useCallback(async (newData: Partial<T>) => {
    ...
    try {
      const result = await hybridDataService.store(table, id, dataWithId, 'create');
  ...
  const update = useCallback(async (id: string, updateData: Partial<T>) => {
    try {
      ...
      const result = await hybridDataService.store(table, id, updatedRecord, 'update');
  ...
  const remove = useCallback(async (id: string) => {
    try {
      const result = await hybridDataService.store(table, id, { deleted: true }, 'delete');
  ...
  // Manual sync
  const syncNow = useCallback(async (direction: SyncDirection = 'bidirectional') => {
    try {
      setError(null);
      console.log(`🔄 Starting manual sync for table: ${table}`);
      
      const result = await hybridDataService.syncBidirectional(table, direction);
      
      if (result.success) {
        console.log(`✅ Sync completed: ${result.localToRemote} to remote, ${result.remoteToLocal} from remote`);
...
export function useHybridOrders() {
  return useHybridData({ table: 'orders', autoSync: true, syncInterval: 15000 });
}

export function useHybridClients() {
  return useHybridData({ table: 'clients', autoSync: true, syncInterval: 30000 });
}

export function useHybridAppointments() {
  return useHybridData({ table: 'appointments', autoSync: true, syncInterval: 10000 });
}

export function useHybridInventory() {
  return useHybridData({ table: 'products', autoSync: true, syncInterval: 60000 });
}

export function useHybridServices() {
  return useHybridData({ table: 'services', autoSync: true, syncInterval: 300000 }); // 5 minutes
} 