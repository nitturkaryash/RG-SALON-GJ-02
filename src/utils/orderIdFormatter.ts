...
export const isSalonConsumptionOrder = (order: OrderForFormatting): boolean => {
  ...
  if (order.services && order.services.length > 0) {
    return order.services.every((service: any) => service.for_salon_use === true);
  }
 ...
 */
export const getDisplayOrderId = (orderId: string, allOrders: any[]): string => {
 ...
 */
export const batchFormatOrderIds = <T extends { id: string }>(orders: T[]): (T & { displayOrderId: string })[] => {
...
export const formatOrderIdForDisplay = (
  ...
  const sameTypeOrders = sortedOrders
    .slice(0, orderIndex + 1)
    .filter(o => {
      const isOtherSalon = o.is_salon_consumption === true || 
 ...
 */
export const formatOrderIdSimple = (orderId: string, orderData?: any): string => {
  ...
  const hashNumber = Math.abs(orderId.split('').reduce((a, b) => {
  ...
  const isSalonOrder = orderData?.is_salon_consumption === true ||
