...
export const sendProfessionalWhatsAppNotification = async (
  ...
  try {
    ...
    console.log(
      `📋 Client: ${appointmentData.clientName}, Booking: ${appointmentData.bookingId}`
    );

    const message = generateMessage(type, appointmentData);
    console.log(
      `🔍 [DEBUG] Generated message length: ${message.length} characters`
    );
...
export const formatAppointmentDataForWhatsApp = (
  ...
  return {
    ...
    bookingId:
      appointmentData.booking_id || `RG${Date.now().toString().slice(-6)}`,
...
export const sendAppointmentConfirmation = async (appointmentData: {
  ...
  return await sendProfessionalWhatsAppNotification(
    'confirmation',
    formattedData
  );
};
...
export const integrateWithAppointmentHooks = {
  ...
  onAppointmentCreated: async (notificationData: {
    ...
    console.log(
      '📱 [Integration] Sending appointment confirmation via professional WhatsApp...'
    );
  ...
  onAppointmentUpdated: async (
    ...
    console.log(
      '📱 [Integration] Sending appointment update via professional WhatsApp...'
    );
    if (isReschedule && oldDate && oldTime) {
      return await sendAppointmentRescheduling(
        notificationData,
        oldDate,
        oldTime
      );
  ...
  onAppointmentCancelled: async (
    ...
    console.log(
      '📱 [Integration] Sending appointment cancellation via professional WhatsApp...'
    );
  ...
  onReminderScheduled: async (
    ...
    console.log(
      `📱 [Integration] Sending ${reminderType} reminder via professional WhatsApp...`
    );
