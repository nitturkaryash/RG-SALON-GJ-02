...
export const setupProductTables = async (): Promise<void> => {
  try {
    ...
    try {
      ...
      throw new Error(
        `Failed to set up UUID extension: ${uuidError instanceof Error ? uuidError.message : 'Unknown error'}`
      );
    }
    ...
    if (errors.length > 0) {
      console.warn(
        `Completed with ${errors.length} non-critical errors:`,
        errors
      );
    } else {
    ...
    try {
      ...
      if (!tablesExist) {
        throw new Error(
          'Tables were not created successfully. Please check the Supabase logs for more details.'
        );
      }
    } catch (verificationError) {
      console.error('Failed to verify tables:', verificationError);
      throw new Error(
        `Tables may have been created but verification failed: ${verificationError instanceof Error ? verificationError.message : 'Unknown error'}`
      );
    }
  } catch (error) {
...
export const checkProductTablesExist = async (): Promise<boolean> => {
  try {
    ...
    console.log('Product collections table check:', {
    ...
    if (collectionsError) {
      console.error(
        'Error checking product_collections table:',
        collectionsError
      );
      return false;
    }
