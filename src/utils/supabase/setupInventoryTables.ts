...
WITH purchase_totals AS (
  SELECT 
    ...
    hsn_code,
    units,
    SUM(purchase_qty) as total_purchases,
    AVG(purchase_taxable_value / NULLIF(purchase_qty, 0)) as avg_cost,
    SUM(purchase_taxable_value) as purchase_value,
    SUM(purchase_cgst) as purchase_cgst,
    SUM(purchase_sgst) as purchase_sgst,
    SUM(purchase_igst) as purchase_igst
...
sales_totals AS (
  SELECT
    ...
    SUM(sales_taxable_value) as sales_value,
    SUM(cgst_rs) as sales_cgst,
    SUM(sgst_rs) as sales_sgst,
    SUM(igst_rs) as sales_igst
  FROM ${TABLES.SALES}
  GROUP BY product_name, hsn_code, units
),
...
consumption_totals AS (
  SELECT
    ...
    SUM(taxable_value) as consumption_value,
    SUM(cgst_rs) as consumption_cgst,
    SUM(sgst_rs) as consumption_sgst,
    SUM(igst_rs) as consumption_igst
...
SELECT
  ...
  p.total_purchases - COALESCE(s.total_sales, 0) - COALESCE(c.total_consumption, 0) as balance_qty,
  p.avg_cost as avg_purchase_cost_per_unit,
  p.purchase_value - COALESCE(s.sales_value, 0) - COALESCE(c.consumption_value, 0) as current_value,
  p.purchase_cgst - COALESCE(s.sales_cgst, 0) - COALESCE(c.consumption_cgst, 0) as current_cgst,
  p.purchase_sgst - COALESCE(s.sales_sgst, 0) - COALESCE(c.consumption_sgst, 0) as current_sgst,
  p.purchase_igst - COALESCE(s.sales_igst, 0) - COALESCE(c.consumption_igst, 0) as current_igst,
  NOW() as last_updated
