...
class HybridDataService {
  ...
  private async tryRemoteSync(
    ...
    try {
      ...
      if (!isResponsive) {
        console.info(
          `📡 Supabase not responsive, keeping ${table}/${id} in sync queue`
        );
        return;
      }
      ...
      switch (operation) {
        ...
        case 'delete':
          supabaseResult = await supabase.from(table).delete().eq('id', id);
          break;
      }
  ...
  async retrieve(
    ...
    try {
      ...
      if (data) {
        ...
        localSuccess = true;
        console.log(
          `📱 Retrieved from local storage: ${table} (${localData.length} records)`
        );
      }
    } catch (error) {
      ...
      try {
        ...
        if (isResponsive) {
          ...
          if (!error && data) {
            ...
            remoteSuccess = true;
            console.log(
              `☁️ Retrieved from Supabase: ${table} (${data.length} records)`
            );
    ...
    localData.forEach(localItem => {
      ...
      if (!remoteItem) {
        ...
        const localTime = new Date(
          localItem.last_modified || localItem.lastModified || 0
        ).getTime();
        const remoteTime = new Date(
          remoteItem.last_modified || remoteItem.lastModified || 0
        ).getTime();
        ...
        for (const record of unsyncedLocal) {
          try {
            ...
            switch (record.operation) {
              case 'create':
                supabaseResult = await supabase.from(table).insert({
                  ...record.data,
              ...
              case 'update':
                supabaseResult = await supabase.from(table).upsert({
                  ...record.data,
                  last_modified: new Date().toISOString(),
                });
                break;
            ...
            if (!supabaseResult.error) {
              ...
              result.errors.push(
                `Failed to sync ${record.id}: ${supabaseResult.error.message}`
              );
            }
          } catch (error) {
          ...
          if (!error && remoteData) {
            for (const item of remoteData) {
              try {
                ...
                result.errors.push(
                  `Failed to store locally ${item.id}: ${error}`
                );
              }
            }
          }
