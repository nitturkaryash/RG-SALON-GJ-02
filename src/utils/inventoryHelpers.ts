  ...
  try {
    ...
    try {
      ...
      if (productsError) {
        ...
        } else if (productsError.message && productsError.message.includes('Invalid API key')) {
          console.error('Authentication error detected with Supabase:', productsError);
          authenticationErrorDetected = true;
        } else {
          console.error('Error fetching from inventory_products:', productsError);
        }
      } else if (productsData && productsData.length > 0) {
        console.log(`Found ${productsData.length} products in inventory_products table`);
    ...
    if (authenticationErrorDetected) {
      console.warn('Skipping additional Supabase queries due to authentication error');
      throw new Error('Authentication error with Supabase. Please check your API keys.');
    }
    ...
    try {
      ...
      if (stockError) {
        if (stockError.code === '42P01') {
          ...
          console.error('Error fetching from inventory_balance_stock:', stockError);
        }
      } else if (stockData && stockData.length > 0) {
        console.log(`Found ${stockData.length} products with stock information`);
      ...
      console.warn('Could not fetch inventory balance, using default stock values:', stockErr);
    }
  ...
  try {
    ...
    for (const item of order.items) {
      ...
      const consumptionRecord = {
        id: `consume-${order.order_id}-${item.product_name}`.replace(/\s+/g, '-'),
      ...
      try {
        ...
        if (insertError) {
          console.warn('Error inserting consumption record, inventory may not be set up:', insertError);
        }
      } catch (err) {
        console.warn('Could not update inventory, it may not be set up yet:', err);
      }
...
export const validateStockForItems = async (items) => {
  try {
    ...
    try {
      ...
      console.warn('Could not check inventory stock, will use products table instead:', err);
    }
    ...
    for (const item of items) {
      ...
      if (
        ...
        (stockMap[productName] !== undefined && stockMap[productName] >= item.quantity)
      ) {
