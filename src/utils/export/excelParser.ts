    ...
    reader.onload = e => {
      try {
        ...
        for (let i = 3; i < rawData.length; i++) {
          ...
          const productNameCol = findColumnName(
            headers,
            COLUMN_MAPPINGS.PRODUCT_NAME
          );
          ...
          const purchaseInvoiceNoCol = findColumnName(
            headers,
            COLUMN_MAPPINGS.PURCHASE_INVOICE_NO
          );
          if (purchaseInvoiceNoCol && row[purchaseInvoiceNoCol]) {
            const purchase: Purchase = {
              ...
              purchase_qty: extractNumber(
                row,
                headers,
                COLUMN_MAPPINGS.PURCHASE_QTY
              ),
              mrp_incl_gst: extractNumber(
                row,
                headers,
                COLUMN_MAPPINGS.MRP_INCL_GST
              ),
              mrp_excl_gst: extractNumber(
                row,
                headers,
                COLUMN_MAPPINGS.MRP_EXCL_GST
              ),
              discount_on_purchase_percentage: extractNumber(
                row,
                headers,
                COLUMN_MAPPINGS.DISCOUNT_PERCENTAGE
              ),
              purchase_cost_per_unit_ex_gst: extractNumber(
                row,
                headers,
                COLUMN_MAPPINGS.PURCHASE_COST_PER_UNIT
              ),
              gst_percentage: extractNumber(
                row,
                headers,
                COLUMN_MAPPINGS.GST_PERCENTAGE
              ),
              purchase_taxable_value: extractNumber(
                row,
                headers,
                COLUMN_MAPPINGS.PURCHASE_TAXABLE_VALUE
              ),
              ...
              invoice_value: extractNumber(
                row,
                headers,
                COLUMN_MAPPINGS.PURCHASE_INVOICE_VALUE
              ),
              created_at: new Date().toISOString(),
              units: units,
          ...
          // Process sales data
          const salesInvoiceNoCol = findColumnName(
            headers,
            COLUMN_MAPPINGS.SALES_INVOICE_NO
          );
          if (salesInvoiceNoCol && row[salesInvoiceNoCol]) {
            const saleQty = extractNumber(
              row,
              headers,
              COLUMN_MAPPINGS.SALES_QTY
            );
            const purchaseCostPerUnit = extractNumber(
              row,
              headers,
              COLUMN_MAPPINGS.PURCHASE_COST_PER_UNIT
            );
            const purchaseGstPercentage = extractNumber(
              row,
              headers,
              COLUMN_MAPPINGS.GST_PERCENTAGE
            );
            const mrpExGst = extractNumber(
              row,
              headers,
              COLUMN_MAPPINGS.MRP_EXCL_GST
            );
            const discountPercentage = extractNumber(
              row,
              headers,
              COLUMN_MAPPINGS.SALES_DISCOUNT
            );
            ...
            if (purchaseGstPercentage > 0) {
              if (purchaseGstPercentage >= 0.1) {
                purchaseCgst =
                  (purchaseTaxableValue * (purchaseGstPercentage / 2)) / 100;
                purchaseSgst =
                  (purchaseTaxableValue * (purchaseGstPercentage / 2)) / 100;
              } else {
            ...
            const sale: Sale = {
              ...
              gst_percentage: extractNumber(
                row,
                headers,
                COLUMN_MAPPINGS.SALES_GST
              ),
              taxable_value: extractNumber(
                row,
                headers,
                COLUMN_MAPPINGS.SALES_TAXABLE_VALUE
              ),
              ...
              invoice_value: extractNumber(
                row,
                headers,
                COLUMN_MAPPINGS.SALES_INVOICE_VALUE
              ),
              created_at: new Date().toISOString(),
            };
          ...
          const consumptionVoucherNoCol = findColumnName(
            headers,
            COLUMN_MAPPINGS.CONSUMPTION_VOUCHER_NO
          );
          if (consumptionVoucherNoCol && row[consumptionVoucherNoCol]) {
            const consumptionQty = extractNumber(
              row,
              headers,
              COLUMN_MAPPINGS.CONSUMPTION_QTY
            );
            const purchaseCostPerUnit = extractNumber(
              row,
              headers,
            ...
            // Calculate derived values
            const taxableValue =
              (consumptionQty || 0) * (purchaseCostPerUnit || 0);
            let cgstValue = 0;
            let sgstValue = 0;
            let igstValue = 0;
            ...
            if ((purchaseGstPercentage || 0) > 0) {
              if ((purchaseGstPercentage || 0) >= 0.1) {
                cgstValue =
                  (taxableValue * ((purchaseGstPercentage || 0) / 2)) / 100;
                sgstValue =
                  (taxableValue * ((purchaseGstPercentage || 0) / 2)) / 100;
              } else {
            ...
            const consumptionItem: Consumption = {
              ...
              requisition_voucher_no:
                row[consumptionVoucherNoCol]?.toString() || '',
        ...
        products.forEach(product => {
          ...
          const totalPurchaseQty = productPurchases.reduce(
            (sum, p) => sum + (p.qty || 0),
            0
          );
          const totalSalesQty = productSales.reduce(
            (sum, s) => sum + (s.qty || 0),
            0
          );
          const totalConsumptionQty = productConsumption.reduce(
            (sum, c) => sum + (c.qty || 0),
            0
          );

          const balanceQty =
            totalPurchaseQty - (totalSalesQty + totalConsumptionQty);

          // Only create balance stock if there's actual stock
          if (balanceQty > 0) {
            const totalPurchaseValue = productPurchases.reduce(
              (sum, p) => sum + (p.taxable_value || 0),
              0
            );
            const avgCostPerUnit =
              totalPurchaseQty > 0 ? totalPurchaseValue / totalPurchaseQty : 0;

            const balanceValue = balanceQty * avgCostPerUnit;
            const balanceGst =
              productPurchases.length > 0
                ? productPurchases[0].gst_percentage || 0
                : 0;
            ...
            balanceStock.push({
              ...
              sgst: balanceSgst,
              invoice_value:
                balanceValue + balanceIgst + balanceCgst + balanceSgst,
