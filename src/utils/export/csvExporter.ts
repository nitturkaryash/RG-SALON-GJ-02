...
// Note: Consumption sync functionality has been moved to useInventory hook
// to avoid duplication and provide better React integration
...
const CSV_HEADERS = [
  ...
  'MRP Incl. GST',
  'MRP Excl. GST',
  'Discount on Sales Percentage',
  'Discounted Sales Rate Excl. GST',
  'Sales GST Percentage',
  'Sales Taxable Value',
  'IGST Rs',
  'CGST Rs',
  'SGST Rs',
...
export const generateCsvData = (data: InventoryExportData): string => {
  ...
  let csvContent =
    'Date,Product Name,HSN Code,Units,Transaction Type,Quantity,';
  csvContent +=
    'Purchase Invoice No,Purchase Price Incl GST,Purchase Price Excl GST,Discount %,';
  csvContent +=
    'Purchase Cost Per Unit Excl GST,Purchase GST %,Purchase Taxable Value,';
  csvContent += 'Purchase CGST,Purchase SGST,';
  csvContent +=
    'Sales Invoice No,MRP Incl GST,MRP Excl GST,Sales Discount %,Discounted Sales Rate,';
  csvContent +=
    'Sales GST %,Sales Taxable Value,Sales IGST,Sales CGST,Sales SGST,Sales Invoice Value,';
  csvContent +=
    'Requisition Voucher No,Consumption Qty,Consumption Cost Per Unit,Consumption GST %,';
  csvContent +=
    'Consumption Taxable Value,Consumption CGST,Consumption SGST,Consumption Total Value,';
...
export function downloadCsv(
  ...
  if (Array.isArray(data)) {
    ...
    if (
      ...
      window.navigator &&
      'msSaveBlob' in window.navigator
    ) {
      // IE/Edge support
