...
import { showToast, handleError, handleSuccess } from '../utils/toastUtils';
...
export async function authenticateWithToken(token: string) {
  try {
    ...
    showToast.success('Authenticated successfully with token');
    return { data: mockUser, error: null };
  } catch (error) {
    console.error('Error authenticating with token:', error);
    handleError(error, 'Failed to authenticate with token');
...
export async function signUp(email: string, password: string) {
  return {
    ...
    error: {
      message:
        'Registration is disabled. Please contact administrators at nitturkashyash@gmail.com or pankajhadole@gmail.com to create an account.',
    },
  };
}
