...
import { Box, Card, CardContent, Grid, Typography, CircularProgress, Chip, Button } from '@mui/material';
...
const InventoryDashboard: React.FC = () => {
  ...
  const fetchStockSummary = async () => {
    ...
    try {
      ...
      if (balanceStock) {
        // Calculate counts
        const outOfStock = balanceStock.filter(item => 
          typeof item.balance_qty === 'number' && item.balance_qty <= 0
        );
        
        const lowStock = balanceStock.filter(item => 
          typeof item.balance_qty === 'number' && 
          item.balance_qty > 0 && 
          item.balance_qty <= 3
        );
        
        const inStock = balanceStock.filter(item => 
          typeof item.balance_qty === 'number' && 
          item.balance_qty > 3
        );
  ...
  useEffect(() => {
    ...
    window.addEventListener('inventory-refresh-requested', handleInventoryUpdate);
    
    return () => {
      window.removeEventListener('inventory-updated', handleInventoryUpdate);
      window.removeEventListener('inventory-refresh-requested', handleInventoryUpdate);
    };
  }, []);
      ...
      <Grid container spacing={2}>
        ...
        <Grid item xs={12} sm={6} md={3}>
          <Card sx={{ bgcolor: 'error.light' }}>
            <CardContent>
              <Typography color="error.dark" gutterBottom>Out of Stock</Typography>
        ...
        <Grid item xs={12} sm={6} md={3}>
          <Card sx={{ bgcolor: 'warning.light' }}>
            <CardContent>
              <Typography color="warning.dark" gutterBottom>Low Stock</Typography>
        ...
        <Grid item xs={12} sm={6} md={3}>
          <Card sx={{ bgcolor: 'success.light' }}>
            <CardContent>
              <Typography color="success.dark" gutterBottom>In Stock</Typography>
        ...
        <Grid item xs={12}>
          <Card>
            <CardContent>
              <Typography variant="h6" gutterBottom>Critical Stock Items</Typography>
                ...
                <Grid container spacing={1}>
                  {stockSummary.lowStockItems.map((item) => (
                    <Grid item key={item.product_name}>
                      <Chip
                        icon={item.balance_qty <= 0 ? <ErrorIcon /> : <WarningIcon />}
