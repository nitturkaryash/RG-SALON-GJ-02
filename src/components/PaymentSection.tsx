...
export const PaymentSection: React.FC<PaymentSectionProps> = ({
  ...
  const handlePaymentAmountChange = useCallback(
    (method: PaymentMethod, value: number) => {
      ...
      if (!isSplitPayment) {
        ...
        const totalEntered = Object.values(updatedPayments).reduce(
          (sum, amt) => sum + amt,
          0
        );
  ...
  useEffect(() => {
    if (!isSplitPayment) {
      const methodsWithAmounts = Object.entries(paymentAmounts).filter(
        ([_, amount]) => amount > 0
      );
  ...
  const totalAmountEntered = Object.values(paymentAmounts).reduce(
    ...
    <Box>
              ...
              <Switch
                ...
                onChange={e => {
                  ...
                  if (!e.target.checked) {
                    ...
                    const primaryMethod =
                      methodsWithAmounts.length > 0
                        ? (methodsWithAmounts[0][0] as PaymentMethod)
                        : 'cash';
      ...
      {isOverpaid && (
        <Alert severity='warning' sx={{ mb: 2 }}>
          Total payment (₹{totalAmountEntered.toLocaleString()}) exceeds bill
          amount by ₹{(totalAmountEntered - totalAmount).toFixed(2)}
        </Alert>
      )}
        ...
        >
          ...
          <Box sx={{ display: 'flex', gap: 1, flexWrap: 'wrap' }}>
            ...
            <Button
              size='small'
              variant={
                paymentAmounts.credit_card > 0 ? 'contained' : 'outlined'
              }
        ...
        >
          ...
          <Box sx={{ display: 'flex', gap: 1, flexWrap: 'wrap' }}>
            ...
            <Button
              size='small'
              variant='outlined'
              onClick={() => quickFillPayment('credit_card')}
              sx={{ minWidth: 'auto', px: 1 }}
            >
              Fill Credit Card
            </Button>
            <Button
              size='small'
              variant='outlined'
              onClick={() => quickFillPayment('debit_card')}
              sx={{ minWidth: 'auto', px: 1 }}
            >
              Fill Debit Card
            </Button>
      ...
      <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 2, mb: 2 }}>
        {(['cash', 'credit_card', 'debit_card', 'upi', 'bnpl'] as const).map(
          method => (
            <Tooltip
              key={method}
              title={PAYMENT_METHOD_DESCRIPTIONS[method]}
              arrow
              placement='top'
            >
              <Box
              ...
              >
                <Box sx={{ display: 'flex', alignItems: 'center', mb: 1 }}>
                  {React.cloneElement(
                    PAYMENT_METHOD_ICONS[method] as React.ReactElement,
                    {
                      sx: {
                        mr: 1,
                        color:
                  ...
                  <Typography
                    variant='body2'
                    fontWeight={paymentAmounts[method] > 0 ? 'bold' : 'normal'}
                  >
                ...
                <TextField
                  ...
                  onChange={e =>
                    handlePaymentAmountChange(method, Number(e.target.value))
                  }
                  InputProps={{
                    startAdornment: (
                ...
                {isSplitPayment && paymentAmounts[method] > 0 && (
                  ...
                  >
                    <Typography variant='caption' color='text.secondary'>
                      {totalAmount > 0
                        ? (
                            (paymentAmounts[method] / totalAmount) *
                            100
                          ).toFixed(1)
                        : 0}
                      %
      ...
      {/* Payment Summary */}
      {Object.entries(paymentAmounts).filter(([_, amount]) => amount > 0)
        .length > 0 && (
        <Box
          sx={{
            p: 1.5,
        ...
        >
          ...
          >
            ...
            <Typography
              variant='body2'
              fontWeight='bold'
              color={isOverpaid ? 'warning.main' : 'text.primary'}
            >
