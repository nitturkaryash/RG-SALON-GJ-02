...
import EnhancedPaymentSection, { PaymentMethod, ActiveMembershipDetails } from './EnhancedPaymentSection';
import { 
...
const DEMO_ORDER_ITEMS: OrderItem[] = [
  { id: '1', name: 'Hair Cut & Styling', price: 1200, quantity: 1, type: 'service' },
  { id: '2', name: 'Hair Wash & Condition', price: 500, quantity: 1, type: 'service' },
  { id: '3', name: 'Premium Hair Serum', price: 800, quantity: 1, type: 'product' },
...
export const SplitPaymentDemo: React.FC = () => {
  ...
  const [paymentValidation, setPaymentValidation] = useState<{isValid: boolean; message?: string}>({isValid: false});
  ...
  const handleCompleteOrder = useCallback(() => {
    ...
    if (!validation.isValid) {
      setSnackbarMessage(`Payment validation failed: ${validation.errors.join(', ')}`);
      return;
    }
  ...
  // Demo scenarios
  const loadDemoScenario = useCallback((scenario: string) => {
    switch (scenario) {
      case 'split_equal':
      ...
      case 'membership_auto_freeze':
        ...
        const maxMembershipUsage = Math.min(total, DEMO_MEMBERSHIP.currentBalance);
  ...
  return (
    <Container maxWidth="lg" sx={{ py: 4 }}>
      ...
      <Typography variant="subtitle1" color="text.secondary" sx={{ textAlign: 'center', mb: 4 }}>
        Experience the modern, smooth split payment functionality inspired by leading POS systems
      </Typography>
              ...
              <Grid item xs={12} sm={6}>
                ...
                <Box sx={{ display: 'flex', gap: 2, mb: 2 }}>
                  <Chip 
                    ...
                    color={totalPaid === total ? 'success' : totalPaid > 0 ? 'warning' : 'default'}
                ...
                {paymentValidation.message && (
                  <Alert severity={paymentValidation.isValid ? 'success' : 'warning'} sx={{ mt: 1 }}>
                    {paymentValidation.message}
                  </Alert>
                )}
              </Grid>
                ...
                {paymentReceipt.payments.map((payment) => (
                  <Box key={payment.id} sx={{ display: 'flex', justifyContent: 'space-between', mb: 1 }}>
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                      {payment.payment_method === 'cash' && <LocalAtm fontSize="small" />}
                      {payment.payment_method === 'credit_card' && <CreditCard fontSize="small" />}
                      {payment.payment_method === 'upi' && <QrCode fontSize="small" />}
                      {payment.payment_method === 'membership' && <AccountBalanceWallet fontSize="small" />}
