    ...
    (acc, item) => {
      ...
      return {
        ...
        current_stock_taxable_value:
          acc.current_stock_taxable_value + current_stock_taxable_value,
        ...
        current_stock_total_value:
          acc.current_stock_total_value + current_stock_total_value,
      };
    },
    {
      quantity: 0,
          ...
          <FormControlLabel
            ...
            label={
              <Typography variant='body2'>
                Show Additional Fields (HSN, Product Type, MRP, etc.)
              </Typography>
            }
          />
          {onExportCsv && (
        ...
        {showAdditionalFields && (
          <>
            ...
            <Box>
              ...
              <Typography variant='subtitle1' fontWeight='bold'>
                {formatCurrency(totals.currentStockAmount)}
              </Typography>
            </Box>
            <Box>
              <Typography variant='caption' color='text.secondary'>
            ...
            <Box>
              ...
              <Typography variant='subtitle1' fontWeight='bold'>
                {formatCurrency(totals.current_stock_taxable_value)}
              </Typography>
            </Box>
            <Box>
              <Typography variant='caption' color='text.secondary'>
              ...
              <Typography variant='subtitle1' fontWeight='bold'>
                {formatCurrency(totals.current_stock_igst)}
              </Typography>
            </Box>
            <Box>
              <Typography variant='caption' color='text.secondary'>
              ...
              <Typography variant='subtitle1' fontWeight='bold'>
                {formatCurrency(totals.current_stock_cgst)}
              </Typography>
            </Box>
            <Box>
              <Typography variant='caption' color='text.secondary'>
              ...
              <Typography variant='subtitle1' fontWeight='bold'>
                {formatCurrency(totals.current_stock_sgst)}
              </Typography>
            </Box>
            <Box>
              <Typography variant='caption' color='text.secondary'>
              ...
              <Typography variant='subtitle1' fontWeight='bold'>
                {formatCurrency(totals.current_stock_total_value)}
              </Typography>
            </Box>
          </>
        )}
                  ...
                  <React.Fragment key={`${row.order_id}-${index}`}>
                    ...
                    >
                      ...
                      <TableCell align='right'>
                        {safeFormatCurrency(row.unit_price_ex_gst)}
                      </TableCell>
                      <TableCell align='center'>
                        <Chip
                      ...
                      <TableCell align='center'>
                        {(() => {
                          ...
                          const discounted =
                            parseFloat(
                              row.discounted_sales_rate_ex_gst || '0'
                            ) || 0;
                          const pct =
                            orig > 0 ? ((orig - discounted) / orig) * 100 : 0;
                          return pct > 0
                      ...
                      <TableCell align='right'>
                        {safeFormatCurrency(
                          parseFloat(row.discounted_sales_rate_ex_gst || '0') *
                            (parseInt(row.quantity, 10) || 0)
                        )}
                      </TableCell>
                      <TableCell align='right'>
                        {safeFormatCurrency(row.taxable_value)}
                      </TableCell>
                      <TableCell align='right'>
                        {safeFormatCurrency(gstValues.cgst)}
                      </TableCell>
                      <TableCell align='right'>
                        {safeFormatCurrency(gstValues.sgst)}
                      </TableCell>

                      {showAdditionalFields && (
                        <>
                          <TableCell align='right'>
                            {safeFormatCurrency(row.igst_amount)}
                          </TableCell>
                          <TableCell>
                            <Typography variant='body2'>
                              {row.hsn_code || '—'}
                          ...
                          <TableCell align='right'>
                            {safeFormatCurrency(row.mrp_incl_gst)}
                          </TableCell>
                          <TableCell align='right'>
                            {safeFormatCurrency(
                              row.discounted_sales_rate_ex_gst
                            )}
                          </TableCell>
                        </>
                      )}
                      ...
                      <TableCell align='right' sx={{ fontWeight: 'bold' }}>
                        {safeFormatCurrency(row.payment_amount)}
                      </TableCell>
                      <TableCell align='right'>
                        {(() => {
                          const orig = parseFloat(row.unit_price_ex_gst) || 0;
                      ...
                      <TableCell align='center'>
                        <Tooltip
                          title={`Stock after transaction: ${row.remaining_stock}`}
                        >
                          <Box
                            sx={{
                              display: 'flex',
                          ...
                          >
                            ...
                            <Chip
                              label={
                                row.remaining_stock !== undefined &&
                                row.remaining_stock !== null
                                  ? row.remaining_stock.toString()
                                  : '—'
                              }
                              size='small'
                              sx={{
                      ...
                      {showAdditionalFields && (
                        <>
                          <TableCell align='right'>
                            {safeFormatCurrency(row.invoice_value)}
                          </TableCell>
                          <TableCell align='center'>
                            <Chip
                              label={
                          ...
                          <TableCell align='right'>
                            {safeFormatCurrency(row.current_stock_amount)}
                          </TableCell>
                          <TableCell align='right'>
                            {safeFormatCurrency(row.c_cgst)}
                          </TableCell>
                          <TableCell align='right'>
                            {safeFormatCurrency(row.c_sgst)}
                          </TableCell>
                          <TableCell align='right'>
                            {safeFormatCurrency(row.c_tax)}
                          </TableCell>
                          <TableCell align='right'>
                            {safeFormatCurrency(
                              row.current_stock_taxable_value
                            )}
                          </TableCell>
                          <TableCell align='right'>
                            {safeFormatCurrency(row.current_stock_igst)}
                          </TableCell>
                          <TableCell align='right'>
                            {safeFormatCurrency(row.current_stock_cgst)}
                          </TableCell>
                          <TableCell align='right'>
                            {safeFormatCurrency(row.current_stock_sgst)}
                          </TableCell>
                          <TableCell align='right'>
                            {safeFormatCurrency(row.current_stock_total_value)}
                          </TableCell>
                        </>
                      )}
                      ...
                      {onDeleteSale && (
                        <TableCell align='center'>
                          <IconButton
                            ...
                            onClick={() =>
                              onDeleteSale &&
                              onDeleteSale(row.order_item_pk || row.id)
                            }
                          ...
                          <Box sx={{ p: 2 }}>
                            ...
                            >
                              ...
                              <Box>
                                ...
                                <Typography variant='body2'>
                                  {safeFormatDate(row.payment_date)}
                                </Typography>
                              </Box>
                              <Box>
                                <Typography
              ...
              >
                ...
                {showAdditionalFields && (
                  <>
                    ...
                    <TableCell align='right'>
                      {formatCurrency(totals.cCgst)}
                    </TableCell>
                    <TableCell align='right'>
                      {formatCurrency(totals.cSgst)}
                    </TableCell>
                    <TableCell align='right'>
                      {formatCurrency(totals.cTax)}
                    </TableCell>
