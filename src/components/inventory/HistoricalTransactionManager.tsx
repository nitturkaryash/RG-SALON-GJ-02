...
const HistoricalTransactionManager: React.FC = () => {
  ...
  const fetchTransactions = async () => {
    ...
    try {
      ...
      const { data: purchases, error: purchasesError } = await supabase
        ...
        .select(
          'purchase_id, product_name, date, purchase_qty, purchase_invoice_number, "Vendor"'
        )
        .eq('user_id', user.id)
      ...
      const { data: sales, error: salesError } = await supabase
        ...
        .select(
          'sale_id, product_name, date, quantity, invoice_no, invoice_value_rs'
        )
        .eq('user_id', user.id)
        .order('date', { ascending: false });
      ...
      const { data: consumption, error: consumptionError } = await supabase
        ...
        .select(
          'consumption_id, product_name, date, consumption_qty, requisition_voucher_no, stylist'
        )
        .eq('user_id', user.id)
        .order('date', { ascending: false });
  ...
  const validateDelete = async (transaction: Transaction) => {
    ...
    try {
      const { data, error } = await supabase.rpc('validate_historical_delete', {
  ...
  const validateUpdate = async (transaction: Transaction) => {
    ...
    try {
      const { data, error } = await supabase.rpc('validate_historical_update', {
        transaction_id_param: transaction.id,
        transaction_type_param: transaction.transaction_type,
  ...
  return (
    <div className='max-w-6xl mx-auto p-6 bg-white rounded-lg shadow-lg'>
      <h2 className='text-2xl font-bold mb-6 text-gray-800'>
        Historical Transaction Manager
      </h2>
            ...
            <div className='mt-3'>
              ...
              <div className='space-y-4'>
                <div>
                  ...
                  <p
                    className={`font-semibold ${validationResult.can_delete || validationResult.can_update ? 'text-green-600' : 'text-red-600'}`}
                  >
                    {validationResult.can_delete || validationResult.can_update
                      ? 'Valid'
                      : 'Invalid'}
                  </p>
                </div>
                ...
                {validationResult.affected_transactions &&
                  validationResult.affected_transactions > 0 && (
                    <div className='p-3 bg-yellow-50 border border-yellow-200 rounded'>
                      <p className='text-yellow-700 text-sm'>
                        ⚠️ This action will affect{' '}
                        {validationResult.affected_transactions} future
                        transaction(s).
                      </p>
                    </div>
                  )}
