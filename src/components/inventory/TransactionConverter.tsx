  ...
  return (
    <Box sx={{ mb: 4 }}>
      ...
      <Typography variant='body2' color='text.secondary' sx={{ mb: 3 }}>
        Select cash sales transactions below and convert them to salon
        consumption to reclassify them for inventory purposes.
      </Typography>
      ...
      {convertSuccess && (
        <Alert severity='success' sx={{ mb: 3 }}>
          ...
          {convertedCount} transaction{convertedCount !== 1 ? 's' : ''}{' '}
          successfully converted to salon consumption.
        </Alert>
      )}
                  ...
                  >
                    <TableCell padding='checkbox'>
                      <Checkbox
                        ...
                        inputProps={{
                          'aria-labelledby': `transaction-${sale.id}`,
                        }}
                    ...
                    <TableCell>
                      <Box>
                        <Typography variant='body2'>
                          {(sale as ExtendedSale).product_name ||
                            'Unknown Product'}
                        </Typography>
                        <Typography variant='caption' color='text.secondary'>
                          HSN: {(sale as ExtendedSale).hsn_code || 'N/A'} |{' '}
                          {(sale as ExtendedSale).unit ||
                            (sale as ExtendedSale).units ||
                            'N/A'}
                        </Typography>
                      </Box>
                    </TableCell>
                    ...
                    <TableCell align='right'>
                      {formatCurrency(sale.invoice_value)}
                    </TableCell>
                    <TableCell align='right'>
                      {sale.igst > 0 ? (
                        <Chip
      ...
      >
        ...
        <Typography variant='body2' sx={{ mt: 1 }}>
          This feature assumes compliance with local tax laws. Converting sales
          to consumption to avoid tax should only be done if legally permitted
          in your jurisdiction. Consult with a tax professional before using
          this feature.
        </Typography>
      </Box>
      ...
      >
        ...
        <DialogContent>
          <DialogContentText id='convert-dialog-description'>
            You are about to convert {selectedTransactions.length} cash sale
            transaction{selectedTransactions.length !== 1 ? 's' : ''} to salon
            consumption. This will reclassify these transactions and remove them
            from sales tax liability. This action cannot be undone.
          </DialogContentText>
        </DialogContent>
        <DialogActions>
      ...
      >
        ...
        <DialogContent>
          <DialogContentText>
            This feature allows you to reclassify cash sales transactions as
            salon consumption for inventory management purposes. When you
          ...
          <Box component='ul' sx={{ pl: 2 }}>
            ...
            <Box component='li' sx={{ my: 1 }}>
              The item remains deducted from inventory, but is now categorized
              as internal use
            </Box>
            <Box component='li' sx={{ my: 1 }}>
              Tax calculations will exclude these transactions from sales tax
              liability
            </Box>
          </Box>
          <DialogContentText sx={{ mt: 2, color: 'warning.dark' }}>
            <strong>Important:</strong> Ensure this practice complies with your
            local tax regulations. Misuse of this feature may lead to tax
            implications.
          </DialogContentText>
        </DialogContent>
        <DialogActions>
