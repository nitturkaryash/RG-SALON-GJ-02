...
export default function StockInsights() {
  ...
  const { loading, error, stats, processStats, fetchInsights, insights } =
    useStockManagement();
  ...
  const [monthlyTrends, setMonthlyTrends] = useState<
    { name: string; purchases: number; sales: number; consumption: number }[]
  >([]);
  ...
  useEffect(() => {
    if (processedData && processedData.topProducts) {
      setTopProducts(
        processedData.topProducts.slice(0, 5).map((p: any) => ({
          name: p.product_name,
          value: p.total_value,
        }))
      );
    ...
    if (processedData && processedData.categoryDistribution) {
      setCategoryDistribution(
        processedData.categoryDistribution.map((c: any) => ({
    ...
    if (processedData && processedData.monthlyTrends) {
      setMonthlyTrends(
        processedData.monthlyTrends.map((m: any) => ({
          name: m.name,
