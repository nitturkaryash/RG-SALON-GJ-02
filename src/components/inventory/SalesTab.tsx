...
const SalesTab: React.FC<SalesTabProps> = ({ sales, isLoading, error }) => {
  console.log('SalesTab rendering with:', { salesCount: sales.length, isLoading, hasError: !!error });
  ...
  const [selectedSale, setSelectedSale] = useState<{ invoiceId: string; productName: string; records: any[] } | null>(null);
  ...
  const exportToCSV = () => {
    try {
      ...
      const csvData = sales.map(sale => {
        ...
        const discountPercentage = sale.discount_on_sales_percentage || sale.discount_percentage || 0;
        const discountedSalesRateExGst = sale.discounted_sales_rate_excl_gst || 0;
        const salesGstPercentage = sale.sales_gst_percentage || sale.gst_percentage || 0;
      ...
      downloadCSV(csvString, `Sales_Data_${new Date().toISOString().split('T')[0]}.csv`);
    } catch (error) {
  ...
  const renderSalesTable = () => {
    ...
    const groupedByInvoice = customerSales.reduce((acc, item) => {
      ...
      const qty = parseFloat(item.quantity?.toString() || item.sales_qty?.toString() || '0');
      const taxableValue = parseFloat(item.taxable_value?.toString() || '0');
      const cgstValue = parseFloat(item.cgst?.toString() || item.cgst_rs?.toString() || '0');
      const sgstValue = parseFloat(item.sgst?.toString() || item.sgst_rs?.toString() || '0');
      const igstValue = parseFloat(item.igst?.toString() || item.igst_rs?.toString() || '0');
      const invoiceValue = parseFloat(item.invoice_value?.toString() || item.invoice_value_rs?.toString() || '0');
      ...
      if (!taxableValue || isNaN(taxableValue)) {
        ...
        const mrpExGst = item.mrp_excl_gst || (item.mrp_incl_gst ? item.mrp_incl_gst / (1 + (item.gst_percentage || 18) / 100) : 0);
        const discountPercentage = item.discount_percentage || item.discount_on_sales_percentage || 0;
      ...
      record.cgst += cgstValue || (calculatedTaxableValue * (item.gst_percentage || 18) / 200); // Half of GST
      record.sgst += sgstValue || (calculatedTaxableValue * (item.gst_percentage || 18) / 200); // Half of GST
          ...
          <Table stickyHeader size="small" aria-label="sales table">
            <TableHead>
              <StyledTableHeaderRow>
                ...
                <StyledTableCell align="right">Purchase Taxable Value (Rs.)</StyledTableCell>
            ...
            <TableBody>
              ...
              <TableRow sx={{ 
                ...
                <TableCell align="right">{customerSales.reduce((sum, item) => sum + parseFloat(item.quantity?.toString() || item.sales_qty?.toString() || '0'), 0).toFixed(2)}</TableCell>
                <TableCell align="right">-</TableCell>
                <TableCell align="right">-</TableCell>
                <TableCell align="right">{formatCurrency(customerSales.reduce((sum, item) => sum + parseFloat(item.purchase_taxable_value?.toString() || '0'), 0))}</TableCell>
                <TableCell align="right">{formatCurrency(customerSales.reduce((sum, item) => sum + parseFloat(item.purchase_igst?.toString() || '0'), 0))}</TableCell>
                <TableCell align="right">{formatCurrency(customerSales.reduce((sum, item) => sum + parseFloat(item.purchase_cgst?.toString() || '0'), 0))}</TableCell>
                <TableCell align="right">{formatCurrency(customerSales.reduce((sum, item) => sum + parseFloat(item.purchase_sgst?.toString() || '0'), 0))}</TableCell>
                <TableCell align="right">{formatCurrency(customerSales.reduce((sum, item) => sum + parseFloat(item.total_purchase_cost?.toString() || '0'), 0))}</TableCell>
              </TableRow>
            </TableBody>
          </Table>
          <TablePagination
  ...
  const handleDeleteSales = async (saleIds: string[]) => {
    try {
      if (!window.confirm(`Are you sure you want to delete ${saleIds.length} sales record(s)?`)) {
        return;
      }
