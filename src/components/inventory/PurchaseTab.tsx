...
const PurchaseTab: React.FC<PurchaseTabProps> = ({
    ...
    const loadInitialData = async () => {
      ...
      try {
        ...
        if (productsData && Array.isArray(productsData)) {
          console.log(
            `Loaded ${productsData.length} products for product name dropdown`
          );
          setExistingProducts(
            productsData.map(p => ({
              id: p.id,
          ...
          console.warn(
            'No products returned from fetchProductsWithStock for product name dropdown'
          );
          setExistingProducts([]);
        }
      } catch (error) {
      ...
      try {
        const { data: productTypeData, error: productTypeError } =
          await supabase.from('products').select('product_type');
        ...
        if (productTypeData) {
          ...
          console.log(
            `Loaded ${distinctTypes.length} distinct product types for units dropdown:`,
            distinctTypes
          );
          setProductTypes(
        ...
        toast.error(
          error instanceof Error
            ? error.message
            : 'Failed to load product types for units dropdown.'
        );
        setProductTypes(['pcs', 'bottles', 'units']);
      } finally {
  ...
  const validateForm = (): boolean => {
    ...
    if (
      formState.discount_on_purchase_percentage < 0 ||
      formState.discount_on_purchase_percentage > 100
    ) {
  ...
  const createProductManually = async () => {
    try {
      ...
      toast.success(
        'Product added locally. Database will be updated when connection is restored.'
      );
      return newProduct;
    } catch (error) {
  ...
  const renderPurchaseTable = () => {
    const groupedByInvoice = purchases.reduce((acc, item) => {
      const invoiceId =
        item.purchase_invoice_number || item.invoice_no || 'Unknown Invoice';
      if (!acc[invoiceId]) {
        acc[invoiceId] = {
          date: item.date,
          products: {},
      ...
      record.purchase_taxable_value += parseFloat(
        item.purchase_taxable_value || '0'
      );
      ...
      record.purchase_invoice_value_rs += parseFloat(
        item.purchase_invoice_value_rs || '0'
      );
      if (record.discount_on_purchase_percentage === 0) {
        record.discount_on_purchase_percentage =
          item.discount_on_purchase_percentage || 0;
      }
    ...
    if (aggregatedRecords.length === 0) {
      return <Alert severity='info'>No purchase records found.</Alert>;
    }
                ...
                ([productName, item]: [string, any], index) => (
                  ...
                  >
                    ...
                    <TableCell align='right'>
                      {item.purchase_qty.toFixed(2)}
                    </TableCell>
                    <TableCell align='right'>
                      {formatCurrency(item.mrp_incl_gst || 0)}
                    </TableCell>
                    <TableCell align='right'>
                      {(item.discount_on_purchase_percentage || 0).toFixed(2)}%
                    </TableCell>
                    <TableCell align='right'>
                      {(item.gst_percentage || 18).toFixed(2)}%
                    </TableCell>
                    <TableCell align='right'>
                      {formatCurrency(item.purchase_taxable_value)}
                    </TableCell>
                    <TableCell align='right'>
                      {formatCurrency(item.purchase_igst)}
                    </TableCell>
                    <TableCell align='right'>
                      {formatCurrency(item.purchase_cgst)}
                    </TableCell>
                    <TableCell align='right'>
                      {formatCurrency(item.purchase_sgst)}
                    </TableCell>
                    <TableCell align='right'>
                      {formatCurrency(item.purchase_invoice_value_rs)}
                    </TableCell>
                    <TableCell>
                      <IconButton
                        size='small'
                ...
                renderInput={params => (
                  <TextField
                    ...
                    helperText={
                      ...
                      (existingProducts.length === 0 && !isLoadingProducts
                ...
                <Select
                  ...
                  disabled={
                    ...
                    existingProducts.some(
                      p => p.name === formState.product_name && p.units
                    )
                  }
                >
