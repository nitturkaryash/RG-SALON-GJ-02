...
function PurchaseForm({ onSubmit, isSubmitting }) {
    ...
    useEffect(() => {
        const loadProducts = async () => {
            try {
                ...
                const [inventoryProductsResult, productCollectionsResult] = await Promise.all([
                ...
                console.log('inventory_products result:', 
                    inventoryProductsResult.error ? 'Error: ' + JSON.stringify(inventoryProductsResult.error) : 
                    `Success: ${inventoryProductsResult.data?.length || 0} products`);
                
                console.log('products result:', 
                    productCollectionsResult.error ? 'Error: ' + JSON.stringify(productCollectionsResult.error) : 
                ...
                if (!inventoryProductsResult.error && inventoryProductsResult.data) {
                    const inventoryProducts = inventoryProductsResult.data.map(product => ({
                    ...
                    console.warn('Error fetching inventory products:', inventoryProductsResult.error);
                }
                
                // Process product collections
                if (!productCollectionsResult.error && productCollectionsResult.data) {
                    const collectionProducts = productCollectionsResult.data.map(product => ({
                    ...
                    console.warn('Error fetching product collections:', productCollectionsResult.error);
                }
                        ...
                        for (const tableInfo of tables) {
                            ...
                            try {
                                ...
                                if (!error && data && data.length > 0) {
                                    console.log(`Found products in table ${tableName}:`, data.length);
                                    
                                    // Try to extract product information from this table
                                    const tableProducts = data.map(item => {
                                        ...
                                        return {
                                            ...
                                            product: {
                                                ...
                                                mrp_incl_gst: parseFloat(item.price || item.mrp_incl_gst || 0),
    ...
    const handleSubmit = (e) => {
        ...
        if (!formData.purchase_qty || formData.purchase_qty <= 0) missingFields.push('purchase_qty');
        if (!formData.mrp_incl_gst || formData.mrp_incl_gst <= 0) missingFields.push('mrp_incl_gst');
        
        if (missingFields.length > 0) {
            console.warn('Missing or invalid fields before validation:', missingFields);
        }
                    ...
                    <Autocomplete
                        ...
                        renderInput={(params) => (
                            <TextField 
                                ...
                                helperText={formErrors.product_name || (productOptions.length === 0 && !isLoadingProducts ? 
...
const PurchaseTab = ({ purchases: initialPurchases, isLoading, error, purchasesQuery }) => {
    ...
    useEffect(() => {
        ...
        console.log('PurchaseTab received purchases:', initialPurchases?.length || 0);
        ...
        if (initialPurchases && initialPurchases.length > 0) {
            setLocalPurchases(prev => {
                ...
                const newPurchases = initialPurchases.filter(p => !existingIds.has(p.purchase_id || p.id));
        ...
        const checkDatabaseConnection = async () => {
            try {
                ...
                if (error) {
                    ...
                    console.log(`Database is connected and responding (${endTime - startTime}ms)`, data);
                ...
                if (purchasesError) {
                    console.error('Cannot access inventory_purchases table:', purchasesError);
                } else {
                    console.log('Successfully accessed inventory_purchases table', purchasesTest);
                }
            } catch (err) {
    ...
    const handleSubmit = async (formData) => {
        ...
        try {
            ...
            if (typeof createPurchase !== 'function') {
                ...
                setDbSaveStatus('error');
                throw new Error('Internal error: Purchase creation function not available');
            }
            ...
            console.log('Purchase created successfully:', newPurchase);
                ...
                try {
                    const { supabase, TABLES } = await import('../../utils/supabase/supabaseClient');
                    
                    // If we have an ID, verify it exists in the database
                    if (newPurchase.id) {
                        console.log('Verifying purchase was saved with ID:', newPurchase.id);
                        ...
                        if (verificationError) {
                            ...
                            alert('Purchase may not have been saved properly. Please check the database.');
                        } else if (!verificationData) {
                            console.error('Verification found no record with ID:', newPurchase.id);
                            alert('Purchase was not found in database after creation. Please try again.');
                        } else {
                            console.log('Verified purchase exists in database:', verificationData);
                        }
                    }
                } catch (verifyError) {
    ...
    const validateFormData = (data) => {
        ...
        if (data.gst_percentage < 0)
            errors.gst_percentage = 'GST percentage cannot be negative';
        if (data.discount_on_purchase_percentage < 0 || data.discount_on_purchase_percentage > 100) {
            errors.discount_on_purchase_percentage = 'Discount must be between 0 and 100%';
        }
            ...
            <Box sx={{ width: '100%' }}>
                ...
                {dbSaveStatus && (
                    <DatabaseSaveIndicator 
                        ...
                        duration={5000}
                        message={
                            dbSaveStatus === 'saving' ? 'Saving purchase to database...' :
                            dbSaveStatus === 'success' ? 'Purchase saved to database successfully!' :
                            dbSaveStatus === 'error' ? 'Failed to save purchase to database. Check console for details.' :
                            null
                        }
                    />
                )}
                ...
                >
                    ...
                    <TablePagination
                        ...
                        component="div"
                        count={localPurchases.length + (cleanedPurchases?.filter(p => 
                            !localPurchases.some(lp => (lp.purchase_id || lp.id) === (p.purchase_id || p.id))
                        ).length || 0)}
