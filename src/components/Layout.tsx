...
const menuLinks: MenuLink[] = [
  ...
  { text: 'Inventory', path: '/inventory', icon: <StoreIcon /> },
  ...
  const userSection = (
    <Box sx={{ p: collapsed ? 1 : 2, borderTop: 1, borderColor: 'divider' }}>
      {!collapsed ? (
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', mb: 2 }}>
            <Avatar sx={{ bgcolor: 'primary.main', mr: 2 }}>
              {session?.username?.charAt(0)?.toUpperCase() ||
                getUsernameFromLocalStorage()?.charAt(0)?.toUpperCase() ||
                'U'}
            </Avatar>
            <Box>
        ...
        >
          <Tooltip
            title={session?.username || getUsernameFromLocalStorage() || 'User'}
            placement='right'
          >
        ...
        {!collapsed && (
          <Typography
            ...
            sx={{
              ...
              background: theme =>
                `linear-gradient(45deg, ${theme.palette.primary.main} 30%, ${theme.palette.success.main} 90%)`,
      ...
      >
        {menuLinks.map(link => (
          <FramerMotion.motion.div
            ...
            style={{ width: '100%' }}
          >
            <ListItemStyled disablePadding sx={{ width: '100%' }}>
              <Tooltip
