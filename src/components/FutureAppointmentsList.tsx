...
const AppointmentCard = ({
  ...
  const getClientName = (appointment: any): string => {
    ...
    if (
      ...
      appointment.clientName !== 'Walk-in Customer'
    ) {
    ...
    if (
      ...
      appointment.client_name !== 'Walk-in Customer'
    ) {
  ...
  const getAppointmentServicesAndStylists = (
    appointment: any,
    allAppointments: any[] = []
  ) => {
    ...
    if (isMultiExpert) {
      ...
      relatedAppointments.forEach(relApp => {
        ...
        if (!relApp.clientDetails || relApp.clientDetails.length === 0) {
          ...
          const serviceName =
            relApp.service_name || relApp.serviceName || 'Unknown Service';
  ...
  const serviceData = getAppointmentServicesAndStylists(
    appointment,
    allAppointments
  );
  ...
  const tooltipContent = (
    <Box sx={{ p: 1, maxWidth: 380, fontSize: '0.875rem' }}>
      ...
      <Box sx={{ mb: 1.5 }}>
        <Typography
          variant='body2'
          sx={{ fontWeight: 'bold', color: 'primary.main', fontSize: '0.9rem' }}
        >
        ...
        {appointment.is_for_someone_else && appointment.booker_name && (
          <Typography
            variant='body2'
            sx={{ color: 'warning.main', fontSize: '0.75rem', mt: 0.5 }}
          >
      ...
      <Box sx={{ mb: 1.5 }}>
        <Typography
          variant='body2'
          sx={{ fontWeight: 'bold', color: 'success.main', fontSize: '0.9rem' }}
        >
        ...
        {serviceData.services.length > 1 ? (
          ...
          <>
            <Typography variant='body2' sx={{ fontSize: '0.8rem', mt: 0.5 }}>
              {serviceData.services[0]?.name} -{' '}
              {formatCurrency(serviceData.services[0]?.price || 0)}
              <span style={{ color: '#666', marginLeft: '8px' }}>
            ...
            >
              ...
              {format(
                new Date(
                  serviceData.services[0]?.startTime || appointment.start_time
                ),
                'hh:mm a'
              )}{' '}
              -{' '}
              {format(
                new Date(
                  serviceData.services[0]?.endTime || appointment.end_time
                ),
                'hh:mm a'
              )}
            </Typography>
      ...
      {serviceData.services.length > 1 && (
        <Box sx={{ mb: 1.5 }}>
          <Typography
            variant='body2'
            sx={{ fontWeight: 'bold', color: 'info.main', fontSize: '0.85rem' }}
          >
            📅 Overall:{' '}
            {format(new Date(appointment.start_time), 'MMM dd, hh:mm a')} -{' '}
          ...
          <Typography
            ...
            sx={{ fontSize: '0.75rem', color: 'text.secondary', mt: 0.5 }}
          >
            {format(new Date(appointment.start_time), 'EEEE')} • Total Duration:{' '}
            {serviceData.totalDuration}min
          </Typography>
        </Box>
      )}
      ...
      {serviceData.services.length === 1 && (
        <Box sx={{ mb: 1.5 }}>
          <Typography
            ...
            sx={{ fontSize: '0.75rem', color: 'text.secondary', mt: 0.5 }}
          >
            {format(new Date(appointment.start_time), 'EEEE, MMM dd')} •
            Duration: {serviceData.totalDuration}min
          </Typography>
        </Box>
      )}
          ...
          <Box sx={{ flexGrow: 1 }}>
            ...
            {serviceData.services.length > 1 ? (
              ...
              <>
                ...
                >
                  {format(
                    new Date(
                      serviceData.services[0]?.startTime ||
            ...
            {serviceData.services.length > 1 && (
              <Typography
                variant='body2'
                color='primary'
                sx={{ mt: 1, fontWeight: 'medium', fontSize: '0.9rem' }}
              >
                Overall: {formattedDateTime} -{' '}
                {format(new Date(appointment.end_time), 'hh:mm a')}
              </Typography>
            )}
          </Box>
        ...
        <TableCell>
          ...
          {serviceData.allStylists.length > 1 && (
            ...
            >
              ...
              {serviceData.allStylists.length > 2 &&
                ` +${serviceData.allStylists.length - 2} more`}
            </Typography>
          )}
        </TableCell>
            ...
            <>
              ...
              {serviceData.services.map((s: any, index: number) => (
                <Typography
                  ...
                  sx={{ mt: 0.5 }}
                >
                  {s.name}: {format(new Date(s.startTime), 'hh:mm a')} -{' '}
                  {format(new Date(s.endTime), 'hh:mm a')}
                </Typography>
              ))}
              <Typography
      ...
      >
        ...
        {useDateRangeFilter && (
          ...
          >
            <Typography variant='body2'>
              ...
              from {format(dateRangeStart, 'MMM dd, yyyy')} to{' '}
              {format(dateRangeEnd, 'MMM dd, yyyy')}
            </Typography>
          </Box>
        )}
        ...
        {filteredAppointments.length > 0 ? (
          viewMode === 'cards' ? (
            ...
            <Box
              sx={{
                ...
                width: '100%',
                justifyContent:
                  filteredAppointments.length === 1 ? 'flex-start' : 'start', // Align single card to left
          ...
          >
            ...
            >
              {tabValue === 1
                ? 'No appointments scheduled for today. Your calendar is clear!'
                : tabValue === 2
                  ? 'No appointments scheduled for tomorrow. Time to book more clients!'
                  : tabValue === 0
