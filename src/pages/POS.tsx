...
import {
  ...
  createWalkInOrder,
  CreateOrderData,
  PaymentMethodWithSplit,
} from '../hooks/orders/usePOS';
...
import { useOrders } from '../hooks/orders/useOrders'; // Import useOrders
...
import { useServices, Service } from '../hooks/products/useServices';
...
import { useAppointments } from '../hooks/appointments/useAppointments'; // <-- Import useAppointments
import {
...
// Define MembershipTier type locally
...
const renderProductCard = (
  product: POSService,
  addHandler: (service: POSService) => void
) => {
    ...
    >
      <CardContent sx={{ flexGrow: 1 }}>
        ...
        <Box sx={{ mb: 2 }}>
          ...
          <Box sx={{ mt: 1 }}>{renderStockBadge(product)}</Box>
        </Box>
      </CardContent>
...
const debugStockQuantity = async (productId: string) => {
  try {
    ...
    console.log(
      `🔍 DEBUG: Product ${data.name} (${productId}) current stock: ${data.stock_quantity}`
    );
...
export default function POS() {
  ...
  const {
    createWalkInOrder: createWalkInOrderMutation,
    createOrder,
    updateOrder: updateOrderMutation,
    orders: posOrders,
    loadingOrders: loadingPOSOrders,
  } = usePOS(); // Renamed orders from usePOS to posOrders
  const { orders, isLoading: isLoadingOrders } = useOrders(); // Orders for lifetime visits calculation
  const { products: inventoryProducts, isLoading: loadingInventoryProducts } =
    useProducts();
  ...
  const { tiers: memberships, isLoading: loadingMemberships } =
    useMembershipTiers();
  ...
  const [selectedStylist, setSelectedStylist] = useLocalStorage<Stylist | null>(
    'pos_selectedStylist',
    null
  );
  ...
  const [serviceDropdownValue, setServiceDropdownValue] =
    useState<Service | null>(null);
  const [productDropdownValue, setProductDropdownValue] =
    useState<POSService | null>(null);
  const [membershipDropdownValue, setMembershipDropdownValue] =
  ...
  const [clientMembershipTier, setClientMembershipTier] = useState<
    string | null
  >(null);
  ...
  const [servicesMembershipPayment, setServicesMembershipPayment] = useState<
    Record<string, boolean>
  >({});
  // Active membership fetched for selected client
  const [activeClientMembership, setActiveClientMembership] =
    useState<ActiveMembershipDetails | null>(null);
  ...
  const [tempSelectedStylists, setTempSelectedStylists] = useState<
    (Stylist | null)[]
  >([]);

  // State for editing existing service items
  const [editingServiceItem, setEditingServiceItem] =
    useState<POSOrderItem | null>(null);

  const [paymentAmounts, setPaymentAmounts] = useLocalStorage<
    Record<PaymentMethod, number>
  >(`${POS_STATE_STORAGE_KEY}_paymentAmounts`, {
    cash: 0,
    credit_card: 0,
    debit_card: 0,
    upi: 0,
  ...
  const handleAddToOrder = useCallback(
    async (service: POSService, quantity: number = 1, customPrice?: number) => {
      console.log(
        `[handleAddToOrder] Called for: ${service.name} (ID: ${service.id}), Quantity: ${quantity}`
      );

      const isOutOfStock =
        service.type === 'product' &&
        typeof service.stock_quantity === 'number' &&
      ...
      console.log(
        `🛒 Adding to order - Product: ${service.name}, ID: ${service.id}, Type: ${service.type}, Quantity: ${quantity}, Current stock: ${service.stock_quantity}`
      );
      ...
      if (
        ...
        console.log(
          `🛒 Stock check - Current cart quantity: ${existingQty}, Adding: ${quantity}, New total: ${newTotalQty}, Available: ${service.stock_quantity}`
        );

        if (newTotalQty > service.stock_quantity) {
          toast.warning(
            `Cannot add ${quantity} of "${service.name}" - Only ${service.stock_quantity} available (${existingQty} already in cart)`
          );
          return;
        }
        ...
        if (remainingStock === 0) {
          toast.warning(
            `Added last ${quantity} of "${service.name}" to cart - No more in stock`
          );
        } else if (remainingStock > 0 && remainingStock <= 3) {
          toast.info(
            `Low stock warning: Only ${remainingStock} ${service.name} remaining after this sale`
          );
        }
      }
      ...
      if (existingItemIndex >= 0) {
        ...
        const itemPrice =
          customPrice !== undefined ? customPrice : existingItem.price; // Keep existing price unless custom price is provided
        ...
        console.log(
          `🔍 POS - Type detection for ${service.name}: original=${service.type}, detected=${itemType}, price=${service.price}`
        );
        ...
        const newItem: POSOrderItem = {
          ...
          total:
            (customPrice !== undefined ? customPrice : service.price) *
            quantity,
          ...
          ...(itemType === 'service' &&
            expertsArray.length > 0 && { experts: expertsArray }),
        };
    ...
    [orderItems, toast]
  ); // Removed selectedStylists since services now get stylists through dialog
      ...
      try {
        ...
        if (ordersData && ordersData.length > 0) {
          ...
          if (itemsData && itemsData.length > 0) {
            itemsData.forEach(item => {
              // Find parent order
              const parentOrder = ordersData.find(
                order => order.id === item.pos_order_id
              );
        ...
        combinedHistory.sort((a, b) => {
          const dateA = new Date(
            a.pos_orders?.created_at || a.appointment_data?.created_at || ''
          );
          const dateB = new Date(
            b.pos_orders?.created_at || b.appointment_data?.created_at || ''
          );
    ...
    (client: Client | null) => {
      ...
      if (client) {
        ...
        if (client.id && client.full_name) {
          ...
          console.warn(
            'Selected client has no id or name, cannot fetch history.'
          );
          setClientServiceHistory([]);
        }
      } else {
  ...
  const calculateProductGstAmount = useCallback(
    (subtotal: number) => {
  ...
  const calculateServiceGstAmount = useCallback(
    (subtotal: number) => {
      // If using membership payment and not split payment, disable GST
      if (useMembershipPayment && !isSplitPayment) {
        return 0;
  ...
  const calculateMembershipGstAmount = useCallback(
    (subtotal: number) => {
      // If using membership payment and not split payment, disable GST
      if (useMembershipPayment && !isSplitPayment) {
        return 0;
  ...
  const calculateTotalAmount = useCallback(() => {
    ...
    const totalAmount =
      orderItems.reduce((sum, item) => {
        const gstPercentage =
          item.gst_percentage ||
          (item.type === 'product' ? productGstRate : serviceGstRate) ||
          18;
        const isGstApplied =
          item.type === 'product'
            ? isProductGstApplied
        ...
        const isServicePaidViaMembership =
          item.type === 'service' && servicesMembershipPayment[item.id];
  ...
  const calculateServicesMembershipTotal = useCallback(() => {
    return orderItems
      .filter(
        item => item.type === 'service' && servicesMembershipPayment[item.id]
      )
      .reduce((sum, item) => {
        const itemTotal = item.price * item.quantity;
        const gstPct = item.gst_percentage || serviceGstRate;
  ...
  const calculateMembershipsRegularTotal = useCallback(() => {
    ...
    const membershipGstAmount =
      calculateMembershipGstAmount(membershipSubtotal);
  ...
  const getRegularPayableAmount = useCallback(() => {
    ...
    return Math.max(
      0,
      servicesRegular + productsRegular + membershipsRegular - globalDiscount
    );
  }, [
    calculateServicesRegularTotal,
    calculateProductsRegularTotal,
    calculateMembershipsRegularTotal,
    walkInDiscount,
  ]);
  ...
  const salonProductsSubtotal = useMemo(() => {
    return salonProducts.reduce((sum, item) => sum + item.total, 0);
  }, [salonProducts]);
  ...
  useEffect(() => {
    if (isSplitPayment) {
      ...
      const maxMembershipPayment = activeClientMembership
        ? Math.min(membershipAmount, activeClientMembership.currentBalance)
        : 0;
  ...
  useEffect(() => {
    ...
    const hasProductsOrMemberships = orderItems.some(
      item => item.type === 'product' || item.type === 'membership'
    );
    const hasServicesPaidViaMembership = Object.values(
      servicesMembershipPayment
    ).some(Boolean);
    ...
    const shouldAutoEnableSplit =
      membershipPayableAmount > 0 &&
      regularPayableAmount > 0 &&
      hasServicesPaidViaMembership &&
      hasProductsOrMemberships;

    if (shouldAutoEnableSplit && !isSplitPayment) {
      console.log(
        '[Auto Split Payment] Enabling split payment: membership services + regular items detected'
      );
    ...
    getMembershipPayableAmount,
    getRegularPayableAmount,
    servicesMembershipPayment,
    orderItems,
    isSplitPayment,
  ...
  const filteredProducts = useMemo(() => {
    const productsOnly = allProducts.filter(
      product =>
  ...
  const serviceHistory = useMemo(() => {
    ...
    return orders.filter(order => {
      ...
      return (
        (orderAny.client_name === selectedClient.full_name ||
  ...
  const filteredMemberships = useMemo(() => {
    ...
    return (memberships || []).filter(
      membership =>
        membership.name
          .toLowerCase()
          .includes(membershipSearchTerm.toLowerCase()) ||
  ...
  useEffect(() => {
    ...
    if (!navigationData) {
      if (processedNavKeyRef.current !== null) {
        console.log(
          '[POS useEffect] No navigation data in location.state. Resetting processedNavKeyRef.'
        );
    ...
    console.log(
      '[POS useEffect] Received navigation data:',
    ...
    if (processedNavKeyRef.current === JSON.stringify(navigationData)) {
      console.log(
        '[POS useEffect] Already processed this navigation data, skipping'
      );
      return;
    }
    ...
    if (loadingServices || !services || services.length === 0) {
      console.log(
        '[POS useEffect] Services not loaded yet, will try again when services load'
      );
      return;
    }
    ...
    if (editOrderData) {
      ...
      const matchingClient = clients?.find(
        c =>
          c.id === editOrderData.clientId ||
          c.full_name === editOrderData.clientName
      );
      if (matchingClient) {
        console.log(
          '[POS useEffect] Setting selected client for edit:',
      ...
      const matchingStylist = stylists?.find(
        s => s.id === editOrderData.stylistId
      );
      if (matchingStylist) {
        console.log(
          '[POS useEffect] Setting selected stylist for edit:',
          matchingStylist.name
        );
        setSelectedStylist(matchingStylist);
      } else {
        console.warn(
          '[POS useEffect] Stylist not found for edit order ID:',
          editOrderData.stylistId
        );
      }
      ...
      if (
        editOrderData.isSalonConsumption &&
        editOrderData.consumptionPurpose
      ) {
      ...
      if (
        editOrderData.services &&
        Array.isArray(editOrderData.services) &&
        editOrderData.services.length > 0
      ) {
        console.log(
          '[POS useEffect] Found',
          editOrderData.services.length,
          'services to edit'
        );

        const newOrderItems = editOrderData.services
          .map((service: any) => {
            console.log('[POS useEffect] Processing edit service:', service);

            const serviceName =
              service.service_name || service.name || service.item_name;
            const serviceId = service.service_id || service.id;

            if (!serviceId || !serviceName) {
              console.warn(
                '[POS useEffect] Invalid service data for edit:',
                service
              );
              return null;
            }
            ...
            console.log(
              '[POS useEffect] Service details lookup:',
              serviceDetails ? 'found' : 'not found'
            );
        ...
        if (newOrderItems.length > 0) {
          console.log(
            '[POS useEffect] Adding edit order items:',
            newOrderItems
          );
      ...
      const matchingClient = clients?.find(
        c => c.full_name === appointmentNavData.clientName
      );
      if (matchingClient) {
        console.log(
          '[POS useEffect] Setting selected client:',
          matchingClient.full_name
        );
        setSelectedClient(matchingClient);
      } else {
        console.warn(
          '[POS useEffect] Client not found for name:',
          appointmentNavData.clientName
        );
      }

      // Find and set stylist(s)
      const matchingStylist = stylists?.find(
        s => s.id === appointmentNavData.stylistId
      );
      if (matchingStylist) {
        console.log(
          '[POS useEffect] Setting selected stylist:',
          matchingStylist.name
        );
        setSelectedStylist(matchingStylist);
      } else {
        console.warn(
          '[POS useEffect] Stylist not found for ID:',
          appointmentNavData.stylistId
        );
      }
      ...
      if (
        appointmentNavData.services &&
        Array.isArray(appointmentNavData.services) &&
        appointmentNavData.services.length > 0
      ) {
        console.log(
          '[POS useEffect] Found service collection with',
          appointmentNavData.services.length,
          'services'
        );

        // Process each service in the collection
        newOrderItems = appointmentNavData.services
          .map((service: any) => {
            ...
            const orderItem: POSOrderItem = {
              ...
              category:
                ...
                'service',
              hsn_code:
                (service as any).hsn_code || (serviceDetails as any)?.hsn_code,
              gst_percentage:
                (service as any).gst_percentage ||
                (serviceDetails as any)?.gst_percentage ||
                18,
              for_salon_use: false,
              discount: 0,
      ...
      console.log(
        '[POS useEffect] Reconstructing expert list from service data...'
      );
      ...
      console.log(
        '[POS useEffect] Experts found in services:',
        expertsInServices
      );
      ...
      if (expertsInServices.length > 1) {
        console.log(
          '[POS useEffect] Setting multiple stylists from service expert data:',
          expertsInServices
        );
        const multipleStylists = expertsInServices.map((expert: any) => {
          ...
          if (!stylistMatch) {
            console.warn(
              '[POS useEffect] Stylist not found for expert ID:',
              expert.id
            );
          }
        ...
        if (stylistMatch) {
          console.log(
            '[POS useEffect] Setting single stylist from service data:',
            stylistMatch.name
          );
        ...
        console.log(
          '[POS useEffect] No experts in services, using fallback stylist'
        );
        setSelectedStylists([]);
      }
      ...
      if (appointmentNavData.id) {
        console.log(
          '[POS useEffect] Setting current appointment ID:',
          appointmentNavData.id
        );
      ...
      try {
        ...
        console.error(
          '[POS useEffect] Error processing appointment data:',
          error
        );
  ...
  const fetchLatestPurchaseCosts = useCallback(async () => {
    try {
      console.log(
        'Fetching latest purchase costs from purchase_history_with_stock...'
      );
      ...
      console.log(
        `Found latest purchase costs for ${Object.keys(latestCosts).length} products`
      );
      return latestCosts;
    } catch (error) {
  ...
  const fetchProductMasterCosts = useCallback(async () => {
    try {
      console.log(
        'Fetching calculated purchase costs from product_master_with_calculated_costs view...'
      );
      ...
      if (data && data.length > 0) {
        for (const item of data) {
          if (item.id && item.final_purchase_cost_ex_gst !== null) {
            ...
            console.log(
              `Found calculated purchase cost for ${item.name} (${item.id}): ₹${item.final_purchase_cost_ex_gst}`
            );
          }
        }
      }
      ...
      console.log(
        `Found product master costs for ${Object.keys(productMasterCosts).length} products`
      );
      return {
        costs: productMasterCosts,
        costsByName: productMasterCostsByName,
      };
    } catch (error) {
  ...
  const [productMasterCosts, setProductMasterCosts] = useState<
    Record<string, number>
  >({});
  const [productMasterCostsByName, setProductMasterCostsByName] = useState<
    Record<string, number>
  >({});

  // Helper function to get purchase cost for a product
  const getPurchaseCostForProduct = useCallback(
    (productId: string, productName: string, fallbackPrice: number): number => {
    ...
    try {
      ...
      if (inventoryProducts && inventoryProducts.length > 0) {
        const mappedProducts = inventoryProducts.map(product => {
          ...
          const mrpExGSTForWalkIn =
            product.mrp_per_unit_excl_gst || product.price || 0;

          console.log(
            `Product ${product.name}: MRP Ex. GST for Walk-In: ${mrpExGSTForWalkIn.toFixed(2)}`
          );
  ...
  // Enhanced validation function that provides specific error messages
  const getValidationErrors = useCallback(() => {
    const errors: string[] = [];

    // Customer info validation
    if (customerName.trim() === '') {
      errors.push('Please enter a client name');
    }
    ...
    if (hasServices) {
      ...
      if (servicesWithoutStylists.length > 0 && selectedStylist === null) {
        errors.push('Please select a stylist for the services');
      }
    }
    ...
    if (!paymentValidation.isValid) {
      if (paymentValidation.isOverpaid) {
        errors.push(
          `Payment exceeds bill amount by ${formatAmount(Math.abs(paymentValidation.remaining))}. Please adjust the payment amounts.`
        );
      } else if (paymentValidation.isUnderpaid) {
        errors.push(
          `Payment incomplete. Remaining amount: ${formatAmount(paymentValidation.remaining)}`
        );
      }
    }
  ...
  useEffect(() => {
    if (isSplitPayment) {
      ...
      const totalEntered = Object.values(paymentAmounts).reduce(
        (sum, amount) => sum + amount,
        0
      );
  ...
  const handlePaymentAmountChange = useCallback(
    (method: PaymentMethod, value: number) => {
      ...
      if (isSplitPayment) {
        ...
        const otherPaymentMethods = Object.entries(paymentAmounts).filter(
          ([m]) => m !== method
        );
        const hasOtherPayments = otherPaymentMethods.some(
          ([_, amt]) => amt > 0
        );
  ...
  const quickFillPayment = useCallback(
  ...
  const handleCreateWalkInOrder = useCallback(async () => {
    ...
    if (membershipPayableAmount > 0 && !activeClientMembership) {
      setSnackbarMessage(
        'Client needs an active membership to pay for selected services via membership.'
      );
      setSnackbarOpen(true);
      return;
    }
    ...
    if (
      membershipPayableAmount > 0 &&
      activeClientMembership &&
      activeClientMembership.currentBalance < membershipPayableAmount
    ) {
      setSnackbarMessage(
        `Insufficient membership balance. Required: ${formatAmount(membershipPayableAmount)}, Available: ${formatAmount(activeClientMembership.currentBalance)}`
      );
      setSnackbarOpen(true);
      return;
    }
    ...
    try {
      ...
      if (!selectedClient && customerName.trim() !== '') {
        ...
        try {
          ...
          if (createdClient && createdClient.id) {
            ...
            toast.success(
              `New client '${clientNameToUse}' created successfully!`
            );
      ...
      console.log(
        `📦 Creating order with ${orderItems.length} total items, including ${productItems.length} products that will reduce inventory:`
      );
      if (productItems.length > 0) {
        productItems.forEach(product => {
          console.log(
            `📦 Product: ${product.item_name}, ID: ${product.item_id}, Quantity: ${product.quantity}`
          );
        });
      }
      ...
      const actualPaymentMethod = isSplitPayment
        ? 'split'
        : walkInPaymentMethod;
      ...
      if (
        ...
        finalClientNameToUpdate !== 'Walk-in Customer'
      ) {
        ...
        console.log(
          'Skipping client update for generic Walk-in Customer or missing name.'
        );
      }
      ...
      const membershipItems = orderItems.filter(
      ...
      const legacyExperts =
        ...
        selectedStylists.every(s => s !== null)
      ...
      const isMembershipOnlyOrder =
        hasMemberships && !hasServices && !hasProducts;
      ...
      const isMultiExpert =
        allExpertsInServices.size > 1 || legacyExperts.length > 1;
      const expertsToProcess =
        allExpertsInServices.size > 0
          ? Array.from(allExpertsInServices)
      ...
      console.log(
        `[Multi-Expert Debug] allExpertsInServices:`,
        Array.from(allExpertsInServices)
      );
      console.log(`[Multi-Expert Debug] isMultiExpert:`, isMultiExpert);
      console.log(
        `[Multi-Expert Debug] expertsToProcess:`,
        expertsToProcess.map(e => e?.name)
      );
      console.log(`[Multi-Expert Debug] numberOfExperts:`, numberOfExperts);

      console.log(
        `[Multi-Expert Order] Creating ${isMultiExpert ? 'single multi-expert' : 'single'} order for ${numberOfExperts} experts:`,
        expertsToProcess.map(e => e?.name)
      );
      ...
      if (isMultiExpert) {
        ...
        const servicesWithExperts = formattedServices.map(service => ({
          ...
          experts:
            (service as any).experts ||
            expertsToProcess.map(expert => ({
              id: expert.id,
              name: expert.name,
            })),
          discount: service.discount || 0,
        ...
        const paymentMethodsUsed = Object.entries(paymentAmounts).filter(
          ([_, amount]) => amount > 0
        );
        const hasMembershipPayment =
          membershipPayableAmount > 0 &&
          paymentMethodsUsed.some(([method]) => method === 'membership');
        const hasRegularPayment =
          regularPayableAmount > 0 &&
          paymentMethodsUsed.some(([method]) => method !== 'membership');
        ...
        const hasProductsOrOtherItems = orderItems.some(
          item => item.type === 'product' || item.type === 'membership'
        );
        ...
        if (hasServicesPaidViaMembership && membershipPayableAmount > 0) {
          const existingMembershipPayment = paymentsArray.find(
            p => p.payment_method === 'membership'
          );
        ...
        if (
          (hasMembershipPayment && hasRegularPayment) ||
          (hasServicesPaidViaMembership && hasProductsOrOtherItems)
        ) {
        ...
        if (!expert && !isNonServiceOrder)
          throw new Error('No expert available for order creation');
      ...
      if (!orderResult.success) {
        ...
        if (errorMessage.includes('Client name is required')) {
          setSnackbarMessage('Please enter a client name');
        } else if (errorMessage.includes('Total amount is required')) {
          setSnackbarMessage('Please add items to the order');
        } else if (errorMessage.includes('Order not found')) {
          setSnackbarMessage('Order not found. Please refresh and try again');
        } else if (errorMessage.includes('Failed to update order items')) {
          setSnackbarMessage(
            'Failed to update order items. Please check your selections'
          );
        } else {
          setSnackbarMessage(
            `Order ${isEditMode ? 'update' : 'creation'} failed: ${errorMessage}`
          );
        }
        setSnackbarOpen(true);
        return;
        ...
        try {
          ...
          for (const result of orderResults) {
            if (result.order && result.order.id) {
              console.log(
                `DEBUG: Updating order ${result.order.id} with appointment_id ${currentAppointmentId}`
              );
              ...
              if (updateOrderError) {
                console.error(
                  'Error updating order with appointment_id:',
                  updateOrderError
                );
              } else {
                console.log(
                  `Successfully linked order ${result.order.id} to appointment ${currentAppointmentId}`
                );
              }
            }
          }
          ...
          console.log(
            `Appointment ${currentAppointmentId} marked as completed, paid, and checked out.`
          );
        } catch (updateError) {
          console.error(
            `Failed to update appointment status for ${currentAppointmentId}:`,
            updateError
          );
          toast.error(
            'Order created, but failed to update appointment status.'
          );
        }
      }
      ...
      const successMessage = isEditMode
        ? isMultiExpert
          ? `Multi-expert order updated successfully for ${numberOfExperts} experts!`
      ...
      if (membershipPayableAmount > 0 && activeClientMembership) {
        ...
        if (error) {
          ...
          toast.success(
            `Membership balance updated. Deducted: ${formatAmount(membershipPayableAmount)}`
          );
        }
      }

      // Initialize balance for new memberships
      const membershipItemsForProcessing = orderItems.filter(
        i => i.type === 'membership' || i.category === 'membership'
      );
        ...
        if (existingMembership) {
          ...
          const { error: updateError } = await supabase
            ...
            .update({
              ...
              expires_at: new Date(
                Date.now() +
                  (item.duration_months || 12) * 30 * 24 * 60 * 60 * 1000
              ).toISOString(),
            })
            .eq('id', existingMembership.id);

          if (updateError) {
            showToast.error('Failed to update membership');
            console.error('Membership update error:', updateError);
          } else {
            showToast.success('Membership renewed');
          }
        } else {
          ...
          if (insertError) {
            showToast.error('Failed to record membership purchase');
    ...
    calculateProductSubtotal,
    calculateServiceSubtotal,
    calculateProductGstAmount,
    calculateServiceGstAmount,
    walkInDiscount,
    getAmountPaid,
    selectedClient,
    selectedStylist,
    customerName,
    createWalkInOrderMutation,
    createOrder,
    isSplitPayment,
    splitPayments,
    productGstRate,
  ...
  const handleAddSalonProduct = useCallback(
    (service: POSService, quantity: number = 1) => {
      ...
      const purchaseCost = getPurchaseCostForProduct(
        service.id,
        service.name,
        service.price
      );
      console.log(
        `Using purchase cost for ${service.name}: ₹${purchaseCost} (from product_master)`
      );
  ...
  const handleCreateSalonConsumptionWorkaround = useCallback(async () => {
    ...
    try {
      ...
      const consumptionDateToUse = orderDate
        ? orderDate.toISOString()
        : currentDate;
      ...
      for (const product of salonProducts) {
        try {
          ...
          if (fetchError) {
            console.error(
              `Error fetching current stock for ${product.item_name}:`,
              fetchError
            );
          ...
          console.log(
            `Current stock for ${product.item_name}: ${currentStock} (No stock reduction applied)`
          );
          ...
          console.log(
            `Salon consumption recorded for ${product.item_name} - Stock remains unchanged at ${currentStock}`
          );
          ...
          const salonConsumptionEntry = {
            'Requisition Voucher No.':
              requisitionVoucherNo || `RV-${Date.now()}`, // Required field
            ...
            'Purchase Taxable Value (Rs.)': parseFloat(
          ...
          if (consumptionError) {
            console.error(
              `Error inserting salon consumption record for ${product.item_name}:`,
              consumptionError
            );
            processResults[processResults.length - 1].success = false;
            processResults[processResults.length - 1].error =
              'Failed to record consumption';
          } else {
            console.log(
              `Added salon consumption record for ${product.item_name} with tax details`
            );
          }
        } catch (err) {
      ...
      const formattedOrderId = await generateNextOrderId(true); // true for salon consumption
      const orderDateToUse = orderDate
        ? orderDate.toISOString()
        : new Date().toISOString();
      const orderTotal = salonProducts.reduce(
      ...
      try {
        ...
        const orderData: any = {
          id: orderId,
          order_id: formattedOrderId, // Add the formatted order ID
        ...
        for (const product of salonProducts) {
          try {
            ...
            console.warn(
              `Failed to get stock for salon consumption product ${product.item_id}:`,
              err
            );
            // Try to get from processResults as fallback
            const result = processResults.find(
              r => r.product === product.item_name
            );
        ...
        if (orderError) {
          ...
          throw new Error(
            `Failed to record consumption order: ${orderError.message}`
          );
        }
        ...
        if (itemsError) {
          ...
          throw new Error(
            `Failed to record consumption order items: ${itemsError.message}`
          );
        }
      ...
      if (typeof queryClient !== 'undefined') {
        ...
        queryClient.invalidateQueries({
          queryKey: ['inventory-salon-consumption'],
        });
        queryClient.invalidateQueries({
      ...
      // Success message
      toast.success(
        'Salon consumption recorded successfully (stock levels unchanged)'
      );
  ...
  const createAppointmentOrder = useCallback(
    async (appointmentId: string) => {
      ...
      try {
        ...
        console.log(
          'Creating appointment order with services:',
          walkInOrderData.services
        );
  ...
  const groupByCategory = (
    items: POSService[] | Service[]
  ): Record<string, (POSService | Service)[]> => {
  ...
  const renderSalonProductSelection = () => {
    ...
    const productsOnly = allProducts.filter(
      product => product.type === 'product' || !product.type
    );
    ...
    return (
      <Box sx={{ p: 3 }}>
        ...
        >
          <Typography variant='h6'>Select Products for Salon Use</Typography>
          <RefreshInventoryButton
            onRefresh={async () => {
              console.log(
                '🔄 POS Salon: Forcefully refreshing product stock data and purchase costs'
              );
              try {
                ...
                toast.success(
                  'Stock data and purchase costs refreshed successfully'
                );
                console.log(
                  '🔄 POS Salon: Stock data and purchase costs refresh complete'
                );
              } catch (error) {
                console.error(
                  '🔄 POS Salon: Error refreshing stock data:',
                  error
                );
          ...
          <Grid item xs={12} sm={6}>
            <TextField
              ...
              onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                setConsumptionPurpose(e.target.value)
              }
          ...
          <Box sx={{ display: 'flex', gap: 2, alignItems: 'center' }}>
            <Autocomplete
              ...
              renderOption={(props, option) => {
                ...
                const actualPurchaseCostFromMaster =
                  productMasterCosts[option.id] ||
                  productMasterCostsByName[option.name.toLowerCase().trim()];
                const priceToDisplay =
                  actualPurchaseCostFromMaster !== undefined
                    ? actualPurchaseCostFromMaster
                    : purchaseCost;
                const priceSource =
                  actualPurchaseCostFromMaster !== undefined
                  ...
                  <li {...props}>
                    <Box>
                      ...
                      <Typography variant='caption' color='text.secondary'>
                        {formatAmount(priceToDisplay)} ({priceSource}) •{' '}
                        {option.stock_quantity !== undefined
                          ? `${option.stock_quantity} in stock`
                          : 'Stock not available'}
                      </Typography>
              ...
              onChange={(_, newProduct) => {
                if (newProduct) {
                  if (
                    ...
                    const purchaseCostForToast = getPurchaseCostForProduct(
                      newProduct.id,
                      newProduct.name,
                      newProduct.price
                    );
                    const actualPurchaseCostFromMasterForToast =
                      productMasterCosts[newProduct.id] ||
                      productMasterCostsByName[
                        newProduct.name.toLowerCase().trim()
                      ];
                    const priceSourceForToast =
                      actualPurchaseCostFromMasterForToast !== undefined
                        ? 'Purchase Cost (Ex.GST)'
                        : `MRP Ex.GST (Fallback)`;
                    toast.success(
                      `Added ${newProduct.name} for salon use at ${formatAmount(purchaseCostForToast)} (${priceSourceForToast})`
                    );
                  }
                }
              }}
        ...
        <Box sx={{ mt: 2 }}>
          ...
          {salonProducts.length === 0 ? (
            <Typography variant='body2' color='text.secondary' sx={{ mt: 1 }}>
              No products selected. Please select products using the dropdown
              above.
            </Typography>
          ) : (
                    ...
                    <TableRow key={product.id}>
                      ...
                      <TableCell align='right'>
                        <IconButton
                          ...
                          color='error'
                          onClick={() =>
                            handleRemoveSalonProduct(product.item_id)
                          }
                        >
      ...
      <Paper sx={{ p: 3, height: '100%' }}>
              ...
              <React.Fragment key={item.id}>
                ...
                >
                  <Box sx={{ flexGrow: 1, mr: 1 }}>
                        ...
                        <Box
                          ...
                          onClick={() => {
                            ...
                            const existingExperts =
                              (item as POSOrderItem).experts || [];
                            const existingStylists = existingExperts.map(
                              expert =>
                          ...
                          if (item.type === 'product') {
                            ...
                            if (
                              ...
                              if (newQuantity > product.stock_quantity) {
                                toast.warning(
                                  `Cannot set quantity to ${newQuantity} - Only ${product.stock_quantity} available in stock`
                                );
                                return;
                              }
                            }
                    ...
                    {item.type === 'service' &&
                      (item as POSOrderItem).experts &&
                      (item as POSOrderItem).experts!.length > 0 && (
                        <Box
                          sx={{
                            mt: 0.5,
                            display: 'flex',
                        ...
                        >
                          {(item as POSOrderItem).experts!.map(
                        ...
                        return (
                          <>
                            <TextField
                              ...
                              onChange={e => {
                                const newPriceIncGst = parseFloat(
                                  e.target.value
                                );
                                // Convert back to ex-GST price for storage
                                const newPriceExGst = isGstApplied
                                  ? newPriceIncGst / gstMultiplier
                                  : newPriceIncGst;
                                handleItemPriceChange(
                                  item.id,
                                  isNaN(newPriceExGst) ? 0 : newPriceExGst
                                );
                              }}
                              sx={{ width: '100px' }}
                              InputProps={{
                            ...
                            >
                                ...
                                onChange={e => {
                                  ...
                                  const updatedItems = orderItems.map(i =>
                                    i.id === item.id
                                      ? {
                                          ...i,
                                          discount: isNaN(discAmount)
                                            ? 0
                                            : discAmount,
                                          // Calculate percentage based on GST-inclusive amount
                                          discount_percentage: isNaN(discAmount)
                                            ? 0
                                            : totalIncludingGst > 0
                                              ? Math.min(
                                                  100,
                              ...
                              <TextField
                                ...
                                onChange={e => {
                                  const discPercentage = parseFloat(
                                    e.target.value
                                  );
                                  const validPercentage = isNaN(discPercentage)
                                    ? 0
                                    : Math.max(
                                        0,
                                        Math.min(100, discPercentage)
                                      );
                                  // Calculate discount amount based on GST-inclusive total
                                  const discAmount =
                                    (validPercentage / 100) * totalIncludingGst;
                                  const updatedItems = orderItems.map(i =>
                                    i.id === item.id
                                      ? {
                                          ...i,
                  ...
                  <Box sx={{ textAlign: 'right', minWidth: '100px' }}>
                    {(() => {
                      ...
                      const isServicePaidViaMembership =
                        item.type === 'service' &&
                        servicesMembershipPayment[item.id];
                      const finalMultiplier = isServicePaidViaMembership
                        ? 1
                        : gstMultiplier;
                      const totalAmount =
                        item.price * item.quantity * finalMultiplier;
                ...
                {item.type === 'service' && (
                  ...
                  >
                        ...
                        <Switch
                          ...
                          onChange={e => {
                            ...
                            if (
                              ...
                              toast.error(
                                'This is a premium service and cannot be purchased with membership balance'
                              );
                              return;
                            }
                            ...
                            if (
                              !activeClientMembership ||
                              !activeClientMembership.isActive
                            ) {
                              toast.error(
                            ...
                            if (e.target.checked) {
                              const gstPercentage =
                                item.gst_percentage || serviceGstRate || 18;
                              const itemTotal = item.price * item.quantity;
                              const gstAmount =
                                (itemTotal * gstPercentage) /
                                (100 + gstPercentage);
                              toast.success(
                                `Membership payment applied! GST discount of ${formatAmount(gstAmount)} automatically applied.`
                              );
                            } else {
                              toast.info(
                                'Membership payment removed. Regular pricing with GST will apply.'
                              );
                            }
                          }}
                          size='small'
                          ...
                          disabled={(() => {
                            ...
                            const isPremiumService =
                              service && service.membership_eligible === false;
                            const noMembership =
                              !activeClientMembership ||
                              !activeClientMembership.isActive;
                    ...
                    {(() => {
                      ...
                      const isPremiumService =
                        service &&
                        (service as any).membership_eligible === false;
                      return (
                        isPremiumService && (
                          <Chip
        ...
        {membershipSubtotal > 0 && (
          <Box sx={{ mb: 1, display: 'flex', justifyContent: 'space-between' }}>
            ...
            <Typography variant='body2' fontWeight='500'>
              {formatCurrency(membershipSubtotal)}
            </Typography>
          </Box>
        )}
        ...
        >
          ...
          <Typography fontWeight='medium'>
            {formatCurrency(serviceGstAmount)}
          </Typography>
        </Box>
        ...
        >
          ...
          <Typography fontWeight='medium'>
            {formatCurrency(productGstAmount)}
          </Typography>
        </Box>
        ...
        {membershipSubtotal > 0 && (
          <Box sx={{ mb: 1, display: 'flex', justifyContent: 'space-between' }}>
            ...
            <Typography variant='body2' fontWeight='500'>
              {formatCurrency(membershipGstAmount)}
            </Typography>
          </Box>
        )}
        {/* Combined GST */}
          ...
          <Box sx={{ mb: 1, ml: 2 }}>
            {individualItemDiscounts > 0 && (
              <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
                ...
                <Typography variant='body2' color='text.secondary'>
                  -{formatCurrency(individualItemDiscounts)}
                </Typography>
              </Box>
            )}
            {walkInDiscount > 0 && (
        ...
        {Object.values(servicesMembershipPayment).some(Boolean) &&
          activeClientMembership && (
            <>
              ...
              >
                <Typography variant='body2' color='primary.main'>
                  Services via Membership (Ex. GST):
                </Typography>
                <Typography
                  variant='body2'
                  fontWeight='500'
              ...
              >
                <Typography variant='body2'>
                  Products & Regular Services (Incl. GST):
                </Typography>
                <Typography variant='body2' fontWeight='500'>
              ...
              >
                <Typography variant='caption' color='text.secondary'>
                  ...
                  />
                  Membership payments exclude GST • Products cannot be paid with
                  membership balance
                </Typography>
              </Box>
            </>
          ...
          <>
                ...
                {formatCurrency(
                  orderItems.reduce((sum, item) => {
                    ...
                    const itemTotalAmount =
                      item.price * item.quantity * gstMultiplier;
                    const finalAmount = Math.max(
                      0,
                      itemTotalAmount - (item.discount || 0)
                    );
                    return sum + finalAmount;
                  }, 0)
                )}
            ...
            >
              ...
              >
                ...
                Membership Discount Applied: GST is excluded when paying via
                membership balance. ₹
                {formatCurrency(getMembershipPayableAmount())} will be deducted
  ...
  const handleAddMembershipToOrder = useCallback(
    (membership: POSMembership) => {
      const newItem: POSOrderItem = {
        id: uuidv4(),
        order_id: '',
  ...
  const isInitialLoading =
    ...
    loadingServices ||
    loadingMemberships ||
    isLoadingOrders; // Added isLoadingOrders
    ...
    <Box sx={{ flexGrow: 1, width: '100%', maxWidth: 1400, mx: 'auto', py: 2 }}>
      ...
      <TabPanel value={tabValue} index={0}>
        <Grid container spacing={2}>
            ...
            <Paper sx={{ p: 2, mb: 2 }}>
              ...
              <Grid container spacing={2}>
                <Grid item xs={12} sm={4}>
                  <Autocomplete
                    ...
                    onInputChange={(_, newInputValue) => {
                      ...
                      if (
                        selectedClient &&
                        !newInputValue.includes(selectedClient.full_name)
                      ) {
                        setSelectedClient(null);
                      }
                    }}
                      ...
                      <TextField
                        ...
                        helperText={
                          clients && clients.length === 0
                            ? 'No clients available. Type a name to add a new one.'
                            : 'Select an existing client or type a new name below.'
                        }
                      />
                    )}
                    renderOption={(props, option) => (
                ...
                <Grid item xs={12} sm={4}>
                  <TextField
                    ...
                    onChange={e => {
                      ...
                      if (
                        selectedClient &&
                        selectedClient.full_name !== e.target.value
                      ) {
                        setSelectedClient(null);
                      }
                    }}
                ...
                <Grid
                  item
                  xs={12}
                  sm={selectedClient && customerName.trim() !== '' ? 4 : 6}
                >
              ...
              >
                <Box sx={{ flex: 1, minWidth: '240px' }}>
                  <Autocomplete
                    ...
                    getOptionLabel={option =>
                      `${option.name} (₹${option.price} • ${option.duration} min)`
                    }
                ...
                <>
                  ...
                  <Accordion
                    ...
                    onChange={(_, isExpanded) =>
                      setClientDetailsExpanded(isExpanded)
                    }
                    sx={{
                      boxShadow: 'none',
                      border: '1px solid',
                    ...
                    <AccordionDetails>
                      <Grid container spacing={2}>
                        ...
                        <Grid item xs={12} sm={6}>
                          ...
                          <Typography>
                            {selectedClient.last_visit
                              ? format(
                                  new Date(selectedClient.last_visit),
                                  'dd/MM/yyyy'
                                )
                              : 'Never'}
                          </Typography>
                        ...
                        <Grid item xs={12} sm={6}>
                          ...
                          {selectedClient.pending_payment &&
                        ...
                        <Grid item xs={12} sm={6}>
                          ...
                          <Typography>
                            {selectedClient.anniversary_date
                              ? format(
                                  new Date(selectedClient.anniversary_date),
                                  'dd/MM/yyyy'
                                )
                              : 'N/A'}
                          </Typography>
          ...
          {/* Order Summary */}
          <Grid item xs={12}>
            <Paper sx={{ p: 2, mb: 2 }}>{renderOrderSummary()}</Paper>
          </Grid>
          ...
          <Grid item xs={12}>
            <Paper sx={{ p: 2, mb: 2 }}>
                ...
                <Grid item xs={12} sm={6}>
                  <TextField
                    ...
                    onChange={e => {
                      ...
                      const subtotalAfterItemDiscounts = orderItems.reduce(
                        (sum, item) =>
                          sum +
                      ...
                      const totalWithGST =
                        ...
                        calculateProductGstAmount(calculateProductSubtotal()) +
                        calculateServiceGstAmount(calculateServiceSubtotal()) +
                        calculateMembershipGstAmount(
                          calculateMembershipSubtotal()
                        );

                      if (totalWithGST > 0) {
                        const calculatedPercentageFromFixed =
                          (fixedDiscount / totalWithGST) * 100;
                        setWalkInDiscountPercentage(
                          Math.max(
                            0,
                            Math.min(
                              100,
                              parseFloat(
                                calculatedPercentageFromFixed.toFixed(2)
                              )
                            )
                          )
                        );
                ...
                <Grid item xs={12} sm={6}>
                  <TextField
                    ...
                    onChange={e => {
                      ...
                      const calculatedFixedDiscountFromPercentage =
                        (totalWithGST * validPercentage) / 100;
                      setWalkInDiscount(
                        parseFloat(
                          calculatedFixedDiscountFromPercentage.toFixed(2)
                        )
                      );
                    }}
                    InputProps={{
              ...
              {isSplitPayment &&
                getMembershipPayableAmount() > 0 &&
                getRegularPayableAmount() > 0 && (
                  <Box
                    sx={{
                      mt: 2,
                      p: 2,
                  ...
                  >
                    ...
                    <Typography variant='body2' color='text.secondary'>
                      Services will be paid via membership balance, and products
                      via regular payment methods.
                    </Typography>
                  </Box>
                )}
              ...
              <Button
                ...
                sx={{ mt: 2 }}
                onClick={async () => {
                  // Check validation first and show specific error messages
                  ...
                  if (isSplitPayment) {
                    ...
                    const activePaymentMethod =
                      (Object.entries(paymentAmounts).find(
                        ([_, amt]) => amt > 0
                      )?.[0] as PaymentMethod) || 'cash';
                ...
                startIcon={
                  processing ? <CircularProgress size={20} /> : <CheckIcon />
                }
              >
                {processing
                  ? 'Processing...'
      ...
      >
        <Box sx={{ width: 400, p: 2 }}>
          ...
          >
            <Typography variant='h6' fontWeight='bold'>
              {selectedClient?.full_name}'s Service History
            </Typography>
            <IconButton
          ...
          {isHistoryLoading ? (
            ...
            >
              <Typography color='text.secondary'>
                No previous services found for this client.
              </Typography>
            </Box>
          ) : (
            <>
                  ...
                  <React.Fragment key={item.id || `item-${index}`}>
                    <ListItem>
                          ...
                          <>
                            <Typography variant='body2'>
                              {item.pos_orders?.created_at
                                ? format(
                                    new Date(item.pos_orders.created_at),
                                    'dd/MM/yyyy'
                                  )
                      ...
                      <Chip
                        size='small'
                        color={
                          item.pos_orders?.status === 'completed' ||
                          item.appointment_data?.status === 'completed'
                            ? 'success'
                            : item.pos_orders?.status === 'cancelled' ||
                                item.appointment_data?.status === 'cancelled'
                              ? 'error'
                              : 'default'
                        }
                        label={
                          item.pos_orders?.status ||
                          item.appointment_data?.status ||
                          'N/A'
                        }
                        sx={{ ml: 1 }}
                      />
      ...
      >
        ...
        <DialogActions>
          ...
          <Button onClick={() => setPrintDialogOpen(false)} variant='outlined'>
            Skip
          </Button>
        </DialogActions>
      </Dialog>
                  ...
                  <FormControl fullWidth size='small'>
                    ...
                    >
                      ...
                      {(stylists || []).map(stylistOption => (
                        <MenuItem
                          key={stylistOption.id}
                          value={stylistOption.id}
                        >
                          {stylistOption.name}
                        </MenuItem>
                      ))}
          ...
          <Button
            onClick={() => {
              if (editingServiceItem) {
                ...
                const validStylists = tempSelectedStylists.filter(
                  s => s !== null
                );
                ...
                const updatedItems = orderItems.map(item =>
                  item.id === editingServiceItem.id
                    ? ({
                        ...item,
                        experts:
                          expertsArray.length > 0 ? expertsArray : undefined,
                      } as POSOrderItem)
                    : item
                );
                setOrderItems(updatedItems);
                ...
                if (expertsArray.length > 0) {
                  const expertNames = expertsArray
                    .map(expert => expert.name)
                    .join(', ');
                  toast.success(
                    `Updated ${editingServiceItem.item_name} stylists: ${expertNames}`
                  );
                } else {
                  toast.success(
                    `Removed all stylists from ${editingServiceItem.item_name}`
                  );
                }
              } else {
                ...
                if (pendingService) {
                  ...
                  if (expertsArray.length > 0) {
                    const expertNames = expertsArray
                      ...
                      .join(', ');
                    toast.success(
                      `Added ${pendingService.name} with stylists: ${expertNames}`
                    );
                  } else {
