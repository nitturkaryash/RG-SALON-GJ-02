...
import CompletePaymentDialog from '../components/modals/CompletePaymentDialog';
...
export default function Orders() {
  ...
  const [isSalonPurchaseFilter, setIsSalonPurchaseFilter] =
    useState<string>('all');
  ...
  const aggregatedOrders = useMemo(() => {
    ...
    Object.entries(ordersByAppointment).forEach(
      ([appointmentId, appointmentOrders]) => {
        if (appointmentOrders.length > 1) {
          // This is a multi-expert appointment - aggregate the orders
          console.log(
            '[Orders Aggregation] Aggregating multi-expert orders for appointment:',
            appointmentId,
            'orders:',
            appointmentOrders.length
          );
          ...
          appointmentOrders.forEach(order => {
            if (order.payments && Array.isArray(order.payments)) {
              order.payments.forEach(p => {
                paymentSummary[p.payment_method] =
                  (paymentSummary[p.payment_method] || 0) + p.amount;
              });
            }
          });
          ...
          appointmentOrders.forEach(order => {
            if (order.services && Array.isArray(order.services)) {
              order.services.forEach((service: any) => {
                ...
                const serviceName =
                  service.name || service.service_name || service.item_name;
                const quantity = service.quantity || 1;
                // Handle different price field names - prioritize 'unitPrice' from actual data
                const price =
                  service.unitPrice || service.price || service.unit_price || 0;
          ...
          finalSubtotal = Object.values(serviceAggregation).reduce(
          ...
          console.log(
            '[Orders Aggregation] Calculated customer-facing total from services. Total:',
            finalTotal,
            'Subtotal:',
            finalSubtotal,
            'Tax:',
          ...
          aggMap[appointmentId] = {
            ...
            tax: finalTax,
            stylist_name: appointmentOrders
              .map(o => o.stylist_name || o.stylist?.name)
              .filter(Boolean)
              .join(', '),
            stylists: appointmentOrders
              .map(o => o.stylist || { name: o.stylist_name, id: o.stylist_id })
              .filter(s => s && s.id),
          ...
          appointmentOrders.forEach(order => {
            if (
              order.stylist_name &&
              !aggMap[appointmentId].stylist_names.includes(order.stylist_name)
            ) {
          ...
          aggMap[appointmentId].stylist_name = aggMap[
            appointmentId
          ].stylist_names
            .filter(Boolean)
            .join(', ');
          ...
          console.log(
            '[Orders Aggregation] Created aggregate for appointment:',
            appointmentId,
            'stylists:',
            aggMap[appointmentId].stylist_name,
            'total:',
            aggMap[appointmentId].total,
            'items:',
            aggMap[appointmentId].services?.length
          );
          ...
          console.log(
            '[Orders Aggregation] Added single appointment order:',
            appointmentOrders[0].id
          );
    ...
    Object.entries(ordersByMultiExpertGroup).forEach(
      ([groupId, groupOrders]) => {
        if (groupOrders.length > 1) {
          // This is a walk-in multi-expert order - aggregate the orders
          console.log(
            '[Orders Aggregation] Aggregating walk-in multi-expert orders for group:',
            groupId,
            'orders:',
            groupOrders.length
          );
          ...
          const tax = groupOrders.reduce(
            (sum, order) => sum + (order.tax || 0),
            0
          );
          const discount = groupOrders.reduce(
          ...
          aggMap[groupId] = {
            ...
            tax: tax,
            discount: discount,
            stylist_name: groupOrders
              .map(o => o.stylist_name || o.stylist?.name)
              .filter(Boolean)
              .join(', '),
            stylists: groupOrders
              .map(o => o.stylist || { name: o.stylist_name, id: o.stylist_id })
              .filter(s => s && s.id),
          ...
          groupOrders.forEach(order => {
            if (
              order.stylist_name &&
              !aggMap[groupId].stylist_names.includes(order.stylist_name)
            ) {
          ...
          // Update stylist_name to show all stylists
          aggMap[groupId].stylist_name = aggMap[groupId].stylist_names
            .filter(Boolean)
            .join(', ');

          console.log(
            '[Orders Aggregation] Created walk-in aggregate for group:',
            groupId,
            'stylists:',
            aggMap[groupId].stylist_name,
            'total:',
            aggMap[groupId].total,
            'items:',
            aggMap[groupId].services?.length
          );
          ...
          console.log(
            '[Orders Aggregation] Added single walk-in multi-expert order:',
            groupOrders[0].id
          );
    ...
    console.log(
      '[Orders Aggregation] Final result count:',
      result.length,
      'from original:',
      orders.length
    );
  ...
  const isSalonConsumptionOrder = (order: ExtendedOrder) => {
    ...
    return (
      ...
      (order.services &&
        order.services.some((s: any) => s.for_salon_use === true)) // Check if any service is marked for salon use
    );
  };
  ...
  const getPurchaseType = (order: ExtendedOrder) => {
    ...
    if (
      ...
      if (
        order.payments &&
        order.payments.some((p: any) => p.payment_method === 'membership')
      ) {
    ...
    const hasMemberships =
      order.services.some(isMembershipService) ||
      ...
      (order.payments &&
        ...
        ) && // Not paid with membership balance
        [1000, 5000, 10000, 15000, 25000, 50000, 100000].includes(
          order.total || order.total_amount || 0
        )) || // Common membership prices
    ...
    const hasServices = order.services.some((service: any) => {
      ...
      const isExplicitProduct =
        service.type === 'product' || service.category === 'product';
      const hasProductAttributes =
        service.product_id ||
        service.product_name ||
        service.hsn_code ||
    ...
    return aggregatedOrders
      ...
      .filter(order => {
        ...
        const matchesSearch =
          ...
          (order.stylist &&
            order.stylist.name &&
            order.stylist.name.toLowerCase().includes(searchLower)) ||
          (order.created_at &&
        ...
        const matchesSalonConsumption =
          ...
          (isSalonPurchaseFilter === 'true' &&
            isSalonConsumptionOrder(order)) ||
          (isSalonPurchaseFilter === 'false' &&
            !isSalonConsumptionOrder(order));
        ...
        if (purchaseTypeFilter === 'all') {
          ...
          matchesPurchaseType =
            orderType === 'both' || orderType === 'service_product_membership';
          ...
          matchesPurchaseType =
            ...
            (order as any)._productCategories.some((cat: string) =>
          ...
          matchesPurchaseType =
            ...
            (order as any)._serviceCategories.some((cat: string) =>
              cat.includes(category)
            );
        }
    ...
    if (filteredOrders) {
      const stats = {
        ...
        completed: filteredOrders.filter(
          order => normalizeOrder(order).status === 'completed'
        ).length,
        pending: filteredOrders.filter(
          order => normalizeOrder(order).status === 'pending'
        ).length,
        salonPurchases: filteredOrders.filter(order =>
          isSalonConsumptionOrder(normalizeOrder(order))
        ).length,
        ...
        totalRevenue: filteredOrders.reduce((sum, orderRaw) => {
          ...
          if (order.status === 'completed') {
            ...
            (order.pending_amount || 0) < (order.total || 0)
          ) {
  ...
  */
  ...
  const confirmDeleteAllOrders = async () => {
    try {
      ...
      const loadingToast = toast.info(
        'Deleting selected date range orders...',
        {
          autoClose: false,
          closeButton: false,
        }
      ...
      const success = await deleteOrdersInDateRange(
        startDate || undefined,
        endDate || undefined
      );
  ...
  const confirmBulkDelete = async () => {
    try {
      const loadingToast = toast.info(
        `Deleting ${selectedOrders.length} selected orders...`,
        {
          autoClose: false,
          closeButton: false,
        }
    ...
    const formattedOrders = sortedOrders.map(order => ({
      ...
      'Services/Products': (() => {
        ...
        if ((order as any).aggregated_multi_expert) {
          const serviceAggregation: Record<
            string,
            { quantity: number; type: string; price: number }
          > = {};
          ...
          return Object.entries(serviceAggregation)
            .map(
              ([name, details]) =>
                `${name} (${details.type}) - Qty: ${details.quantity}`
            )
            .join('; ');
        }
          ...
          if (isAggregatedOrder) {
            ...
            order.payments.forEach(payment => {
              ...
              paymentSummary[method] =
                (paymentSummary[method] || 0) + payment.amount * expertCount;
            });
          } else {
            order.payments.forEach(payment => {
              const method = payment.payment_method;
              paymentSummary[method] =
                (paymentSummary[method] || 0) + payment.amount;
            });
          }

          return Object.entries(paymentSummary)
            .map(
              ([method, amount]) =>
      ...
      'Subtotal (Ex. Tax)': (() => {
        ...
        if (isAggregatedOrder && order.services && order.services.length > 0) {
          ...
          subtotal = Object.values(serviceAggregation).reduce(
            (sum, item) => sum + item.total_price,
            0
          );
    ...
    formattedOrders.forEach((order, index) => {
      ...
      switch (type) {
        ...
        case 'membership':
          ...
          break;
        case 'both':
          order['Type'] = 'Mixed (Service & Product)';
          break;
        case 'service_membership':
          order['Type'] = 'Mixed (Service & Membership)';
          break;
        case 'product_membership':
          order['Type'] = 'Mixed (Product & Membership)';
          break;
        case 'service_product_membership':
          order['Type'] = 'Mixed (Service & Product & Membership)';
          break;
        default:
          order['Type'] = 'Unknown';
          break;
  ...
  return (
    <Container maxWidth='lg'>
      <Box sx={{ mb: 4 }}>
          ...
          >
            ...
            <Tab
              ...
              sx={{
                ...
                fontWeight:
                  activeTab === OrderTab.SALON_CONSUMPTION ? 600 : 400,
              }}
            />
            <Tab
            ...
            <Grid container spacing={2}>
              ...
              <Grid item xs={12} md={8}>
                <Grid container spacing={2}>
                  ...
                  <Grid item xs={12} sm={6} md={4}>
                    <FormControl fullWidth size='small'>
                      ...
                      >
                        ...
                        <MenuItem value='product:retail'>
                          Retail Products
                        </MenuItem>
                        <MenuItem value='product:salon'>
                          Salon Products
                        </MenuItem>
                        <Divider sx={{ my: 1 }} />
                        <Typography
                  ...
                  <Grid item xs={12} sm={6} md={4}>
                    <FormControl fullWidth size='small'>
                      <InputLabel id='salon-purchase-filter-label'>
                        Salon Consumption
                      </InputLabel>
                      <Select
          ...
          <AccessibleDialog
            ...
            actions={
              <>
                <Button
                  onClick={() =>
                    selectedOrder && handlePrintBill(selectedOrder)
                  }
                  variant='contained'
                  color='secondary'
                  ...
                  <Typography variant='body2'>
                    <strong>
                      ...
                      {(() => {
                        // Check if this is a multi-expert order by looking at services
                        const allStylistsFromServices = new Set<string>();
                        (selectedOrder.services || []).forEach(
                          (service: any) => {
                            // Add from legacy stylist_name field
                            if (service.stylist_name)
                  ...
                  <Typography variant='body2'>
                    ...
                    selectedOrder.payments.length > 0
                      ? selectedOrder.payments
                          ...
                          .join(', ')
                      : PAYMENT_METHOD_LABELS[
                          selectedOrder.payment_method as PaymentMethod
                        ] ||
              ...
              <Grid item xs={12} md={6}>
                ...
                <Box sx={{ mb: 2 }}>
                  {(() => {
                    ...
                    const isAggregatedMultiExpert = (selectedOrder as any)
                      .aggregated_multi_expert;
                        ...
                        {isAggregatedMultiExpert && (
                          ...
                          >
                            ...
                            >
                              ...
                              />
                              This is a multi-expert order. Amounts shown are
                              the total for all services across all experts.
                            </Typography>
                          </Box>
                        )}
                  ...
                  {(() => {
                    ...
                    if (hasMembershipPayment && membershipAmount > 0) {
                      ...
                      const estimatedMembershipGSTDiscount =
                        (membershipAmount * 0.18) / 1.18;
                      ...
                      return (
                        <>
                          ...
                          >
                            ...
                            >
                              <InfoIcon
                                ...
                                sx={{ verticalAlign: 'middle', mr: 0.5 }}
                              />
                              Membership payments exclude GST • Products cannot
                              be paid with membership balance
                            </Typography>
                          </Box>
                          ...
                          >
                            ...
                            <Typography variant='h6' fontWeight='bold'>
                              {formatAmount(
                                totalOrderAmount + membershipAmount
                              )}
                            </Typography>
                          </Box>
                          ...
                          >
                            ...
                            >
                              ...
                              Membership Discount Applied: GST is excluded when
                              paying via membership balance. ₹
                              {formatCurrency(membershipAmount)} was deducted
                            ...
                            <TableBody>
                              {(() => {
                                ...
                                if (isAggregatedMultiExpert) {
                                  ...
                                  const aggregatedPayments: Record<
                                    string,
                                    number
                                  > = {};
                                  selectedOrder.payments.forEach(
                                    (payment: any) => {
                                      const method = payment.payment_method;
                                  ...
                                  return Object.entries(aggregatedPayments).map(
                                    ([method, amount]) => (
                                      <TableRow key={method}>
                                        <TableCell>
                                          <Box
                                  ...
                                  return selectedOrder.payments.map(
                  ...
                  <Table size='small'>
                    ...
                    <TableBody>
                      {(() => {
                        ...
                        (selectedOrder.services || []).forEach(
                          (service: any) => {
                            ...
                            if (!serviceAggregation[serviceName]) {
                              serviceAggregation[serviceName] = {
                                ...
                                benefit_amount:
                                  service.benefit_amount ||
                                  service.benefitAmount,
                                paid_with_membership:
                                  service.paid_with_membership,
                              };
                            }
                            ...
                            if (
                              ...
                              !serviceAggregation[
                                serviceName
                              ].stylists.includes(stylistName)
                            ) {
                              serviceAggregation[serviceName].stylists.push(
                                stylistName
                              );
                            }
                        ...
                        return Object.values(serviceAggregation).map(
                          (aggregatedService, index) => (
                            <TableRow key={index}>
                              <TableCell>
                              ...
                              <TableCell>
                                <Chip
                                  ...
                                  color={(() => {
                                    if (aggregatedService.type === 'product')
                                      return 'secondary';
                                    if (
                                      aggregatedService.type === 'membership' ||
                                      (aggregatedService.service_name &&
                                ...
                                <Typography variant='body2' fontWeight='medium'>
                                  ...
                                  {aggregatedService.stylists.length > 1 && (
                                    <Typography
                                      ...
                                      color='text.secondary'
                                    >
                                      ({aggregatedService.stylists.length}{' '}
                                      experts)
                                    </Typography>
                                  )}
                                </Typography>
                              ...
                              <TableCell align='center'>
                                  ...
                                  >
                                    ...
                                    {aggregatedService.benefit_amount && (
                                      ...
                                      >
                                        Benefit: ₹
                                        {(
                                          aggregatedService.benefit_amount || 0
                                        ).toLocaleString()}
                                      </Typography>
                                    )}
                                  </Box>
                                ...
                                {aggregatedService.type === 'service' &&
                                  aggregatedService.paid_with_membership && (
                                    <Box
                                      sx={{
                                        display: 'flex',
                                        flexDirection: 'column',
        ...
        >
          ...
          <DialogContent>
            <DialogContentText id='alert-dialog-description-all'>
              Warning: This will permanently delete ALL orders from{' '}
              {startDate ? startDate.toLocaleDateString() : 'the beginning'} to{' '}
        ...
        >
          <DialogTitle id='alert-dialog-title-bulk'>
            {'Delete Selected Orders'}
          </DialogTitle>
          <DialogContent>
            <DialogContentText id='alert-dialog-description-bulk'>
              Warning: This will permanently delete {selectedOrders.length}{' '}
              selected order{selectedOrders.length !== 1 ? 's' : ''}. This
