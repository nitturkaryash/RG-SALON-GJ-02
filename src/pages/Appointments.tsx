...
} from '../hooks/products/useServices';
...
interface ExtendedClientAppointmentEntry
  extends Omit<ClientAppointmentEntry, 'services'> {
  services: ServiceEntry[];
...
const buttonSx = (
  type: 'male' | 'female',
  currentFilter: 'male' | 'female' | null
) => ({
  minWidth: '80px',
  borderRadius: '20px',
  textTransform: 'none',
...
const CustomAutocompleteListbox = React.forwardRef<
  HTMLUListElement,
  CustomListboxProps
>((props, ref) => {
  const {
    children,
    serviceGenderFilterState,
    setServiceGenderFilterState,
    ...other
  } = props;
  return (
      ...
      >
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
          ...
          <Button
            size='small'
            variant={
              serviceGenderFilterState === null ? 'contained' : 'outlined'
            }
            ...
            sx={{
              ...
              textTransform: 'none',
              bgcolor:
                serviceGenderFilterState === null ? '#6B8E23' : 'transparent',
              color: serviceGenderFilterState === null ? 'white' : '#757575',
              borderColor: '#E0E0E0',
              '&:hover': {
                bgcolor:
                  serviceGenderFilterState === null
                    ? '#566E1C'
                    : 'rgba(0,0,0,0.04)',
              },
            }}
          ...
          <Button
            size='small'
            variant={
              serviceGenderFilterState === 'female' ? 'contained' : 'outlined'
            }
            onClick={() =>
              setServiceGenderFilterState(
                serviceGenderFilterState === 'female' ? null : 'female'
              )
            }
          ...
          <Button
            size='small'
            variant={
              serviceGenderFilterState === 'male' ? 'contained' : 'outlined'
            }
            onClick={() =>
              setServiceGenderFilterState(
                serviceGenderFilterState === 'male' ? null : 'male'
              )
            }
...
export default function Appointments() {
  ...
  const [clientEntries, setClientEntries] = useState<
    ExtendedClientAppointmentEntry[]
  >([
    {
      id: uuidv4(),
      client: null,
  ...
  const [showInlineServiceSelection, setShowInlineServiceSelection] =
    useState(false);
  ...
  const [isInlineServiceDropdownOpen, setIsInlineServiceDropdownOpen] =
    useState(false); // New state for inline Autocomplete
  // Removed clientDialogOpen state
  // Removed serviceDialogOpen state
  const [selectedEntryId, setSelectedEntryId] = useState<string>('');
  const [serviceSelectedMessage, setServiceSelectedMessage] =
    useState<string>('');
  ...
  const [drawerDatePickerAnchorEl, setDrawerDatePickerAnchorEl] =
  ...
  const [lastBookedAppointmentTime, setLastBookedAppointmentTime] = useState<
    string | undefined
  >(undefined);
  ...
  const updateEntry = (
    id: string,
    updates: Partial<ExtendedClientAppointmentEntry>
  ) => {
    setClientEntries(prev =>
      prev.map(entry => {
        if (entry.id === id) {
          ...
          if (updates.services && Array.isArray(updates.services)) {
            const services = updates.services as unknown as
              | ServiceWithGender[]
              | ServiceEntry[];
            // Check if the first item is a Service by seeing if it lacks ServiceEntry properties
            if (
              services.length > 0 &&
              !('stylistId' in services[0]) &&
              !('fromTime' in services[0])
            ) {
              // Convert Service objects to ServiceEntry objects
              const convertedServices: ServiceEntry[] = (
                services as ServiceWithGender[]
              ).map(service => ({
                id: service.id,
                name: service.name,
  ...
  const handleAppointmentClick = (appointment: MergedAppointment) => {
    console.log(
      '[handleAppointmentClick] Received appointment object:',
    ...
    if (!primaryClientInfo) {
      console.warn(
        `[handleAppointmentClick] Primary client data not found for ID: ${appointment.client_id}. Cannot populate drawer.`
      );
      setClientEntries([
        {
          id: uuidv4(),
    ...
    console.log(
      '[handleAppointmentClick] Primary client found:',
      primaryClientInfo
    );
    ...
    console.log(
      '[handleAppointmentClick] Found group appointments:',
      groupAppointments.map(a => a.id)
    );
    ...
    groupAppointments.forEach((app: MergedAppointment) => {
      ...
      if (
        app.clientDetails &&
        Array.isArray(app.clientDetails) &&
        app.clientDetails.length > 0
      ) {
        app.clientDetails.forEach(clientDetailEntry => {
          if (
            clientDetailEntry.id === primaryClientInfo.id &&
            Array.isArray(clientDetailEntry.services)
          ) {
            clientDetailEntry.services.forEach(serviceFromDetail => {
              const serviceId = serviceFromDetail.id;
              const serviceStylistId =
                (serviceFromDetail as any).original_stylist_id ||
                (serviceFromDetail as any).stylist_id ||
                app.stylist_id;
              const serviceStartTimeStr =
                (serviceFromDetail as any).original_start_time ||
                (serviceFromDetail as any).start_time ||
                app.start_time;
              const serviceEndTimeStr =
                (serviceFromDetail as any).original_end_time ||
                (serviceFromDetail as any).end_time ||
                app.end_time;
              ...
              if (
                clientDetailEntry.stylists &&
                clientDetailEntry.stylists.length > 0
              ) {
                clientDetailEntry.stylists.forEach(stylist => {
                  if (
                    stylist &&
                    stylist.id &&
                    !serviceEntry.experts.find(e => e.id === stylist.id)
                  ) {
                    serviceEntry.experts.push({
                      id: stylist.id,
                      name: stylist.name,
                    });
                  }
    ...
    if (consolidatedServiceEntries.length > 0) {
      ...
      const allEndTimes = consolidatedServiceEntries.map(s => {
        ...
        const startOfDayForService = new Date(d);
        startOfDayForService.setHours(0, 0, 0, 0);
        const serviceStartTimeDate =
          allStartTimes[
            consolidatedServiceEntries.findIndex(
              entry => entry.toTime === s.toTime
            )
          ];
        if (d < serviceStartTimeDate && serviceStartTimeDate)
    ...
    console.log(
      '[handleAppointmentClick] Drawer populated. Services count:',
      consolidatedServiceEntries.length,
      'Overall Times:',
  ...
  const handleDayViewSelect = (stylistId: string, time: Date) => {
    ...
    if (!isSameStylist) {
      console.log(
        'Updating selectedStylistId from',
        selectedStylistId,
        'to',
        stylistId
      );
    ...
    if (
      ...
      clientEntries.some(entry => entry.services.length > 0)
    ) {
      ...
      const updatedEntries = clientEntries.map(entry => ({
        ...entry,
        stylists: preSelectedStylist
          ? [{ id: preSelectedStylist.id, name: preSelectedStylist.name }]
          : entry.stylists,
      }));
      setClientEntries(updatedEntries);
    } else {
      console.log(
        'Creating new client entry for',
        isSameStylist
          ? 'same stylist (no existing services)'
          : 'different stylist'
      );
  ...
  const handleCheckServiceSelectionBeforeDialog = (entryId: string) => {
    ...
    const hasMissingStylist = entry?.services.some(
      service => !service.stylistId
    );

    if (hasMissingStylist) {
      toast.warn(
        'Please assign an expert to all services before adding a new one'
      );
      return;
    }
    ...
    const serviceSearchInput = document.querySelector(
      'input[placeholder="Search Service By Name"]'
    ) as HTMLInputElement;
  ...
  const removeServiceFromEntry = (entryId: string, serviceIndex: number) => {
    ...
    for (let i = serviceIndex; i < updatedServices.length; i++) {
      ...
      const [fromHour, fromMinute] = updatedServices[i].fromTime
        .split(':')
        .map(Number);
      ...
      updatedServices[i].toTime =
        `${toHour}:${toMinute.toString().padStart(2, '0')}`;
    }
  ...
  const updateServiceStylist = (
    ...
    updatedServices[serviceIndex] = {
      ...
      experts: updatedServices[serviceIndex].experts || [
        { id: stylist.id, name: stylist.name },
      ],
    };
  ...
  const updateServiceTime = (
    ...
    if (field === 'fromTime' && serviceIndex < updatedServices.length - 1) {
      ...
      const [toHour, toMinute] = updatedServices[serviceIndex].toTime
        .split(':')
        .map(Number);
      ...
      updatedServices[serviceIndex].toTime =
        `${newToHour}:${newToMinute.toString().padStart(2, '0')}`;
      ...
      for (let i = serviceIndex + 1; i < updatedServices.length; i++) {
        ...
        const [nextFromHour, nextFromMinute] = updatedServices[i].fromTime
          .split(':')
          .map(Number);
        const service = activeServices.find(
          s => s.id === updatedServices[i].id
        );
        const serviceDuration = service?.duration || 60;
        ...
        updatedServices[i].toTime =
          `${nextToHour}:${nextToMinute.toString().padStart(2, '0')}`;
      }
    }
    ...
    if (field === 'toTime' && serviceIndex < updatedServices.length - 1) {
      ...
      for (let i = serviceIndex + 1; i < updatedServices.length; i++) {
        ...
        const totalMinutes =
          nextFromHour * 60 + nextFromMinute + serviceDuration;
        const nextToHour = Math.floor(totalMinutes / 60) % 24; // Use modulo to keep in 24h format
        const nextToMinute = totalMinutes % 60;
  ...
  const handleSaveAppointment = async () => {
    if (!editingAppointment) {
      ...
      try {
        ...
        const appointmentsToCreate = primaryEntry.services
          .map(service => {
            ...
            const expertsList = rawExpertsList.filter(
              e => e.id && e.id.trim() !== ''
            );
            if (expertsList.length === 0) return null; // skip if no valid expert

            const [startHour, startMinute] = service.fromTime
              .split(':')
              .map(Number);
            ...
            return {
              ...
              status:
                (selectedAppointmentStatus as Appointment['status']) ||
                'scheduled',
              ...
              clientDetails: [
                {
                  ...
                  stylistIds: expertsList
                    .map(e => e.id)
                    .filter((id): id is string => Boolean(id)), // ALL experts for this service
                },
              ],
        ...
        if (isWhatsAppEnabled()) {
          ...
          if (
            primaryEntry.client &&
            primaryEntry.client.phone &&
            isValidPhoneNumber(primaryEntry.client.phone)
          ) {
            try {
              const firstService = primaryEntry.services[0];
              if (firstService) {
                // Calculate the start and end times for the first service
                const [startHour, startMinute] = firstService.fromTime
                  .split(':')
                  .map(Number);
                const [endHour, endMinute] = firstService.toTime
                  .split(':')
                  .map(Number);
                const baseDate = new Date(drawerDate);
                ...
                console.log(
                  '[WhatsApp] Appointment confirmation notification sent successfully'
                );
              }
            } catch (whatsappError) {
              console.error(
                '[WhatsApp] Failed to send appointment confirmation notification:',
                whatsappError
              );
              // Don't block the appointment creation if WhatsApp fails
              toast.warning(
                'Appointment created successfully, but WhatsApp notification failed to send'
              );
            }
          } else {
            console.log(
              '[WhatsApp] Skipping confirmation notification - invalid or missing phone number'
            );
          }
        }
      ...
      try {
        const existingAppointmentDbData = allAppointments.find(
          app => app.id === editingAppointment.id
        );
        const bookingId =
          existingAppointmentDbData?.booking_id ||
          editingAppointment.booking_id ||
          uuidv4();
        ...
        if (isSimpleUpdate) {
          ...
          console.log(
            '[UPDATE] Performing direct update for simple appointment change'
          );
          ...
          const existingExperts = service.experts
            ?.map(e => e.id)
            .filter(Boolean) || [service.stylistId];
          const newExperts = service.experts
            ?.map(e => e.id)
            .filter(Boolean) || [service.stylistId];
          const expertsChanged =
            JSON.stringify(existingExperts.sort()) !==
            JSON.stringify(newExperts.sort());
            ...
            updateData.clientDetails = [
              {
                clientId: primaryEntry.client.id,
                serviceIds: [service.id],
                stylistIds: newExperts.filter((id): id is string =>
                  Boolean(id)
                ),
              },
            ];
          ...
          console.log(
            '[UPDATE] Performing complex update with delete-recreate approach'
          );
          ...
          console.log(
            '[UPDATE] Found related appointments to delete:',
            relatedAppointments.map(a => a.id)
          );

          // Delete all existing related appointments
          await Promise.all(
            relatedAppointments.map(app => deleteAppointment(app.id))
          );
          ...
          const appointmentsToCreate = primaryEntry.services
            .map(service => {
              ...
              const [startHour, startMinute] = service.fromTime
                .split(':')
                .map(Number);
              const [endHour, endMinute] = service.toTime
                .split(':')
                .map(Number);
              const baseDate = new Date(drawerDate);
          ...
          console.log(
            '[UPDATE] Created new appointments:',
            appointmentsToCreate.length
          );
        }
          ...
          if (
            ...
            try {
              ...
              if (firstService && editingAppointment) {
                ...
                console.log(
                  '[WhatsApp] Appointment update notification sent successfully'
                );
              }
            } catch (whatsappError) {
              console.error(
                '[WhatsApp] Failed to send appointment update notification:',
                whatsappError
              );
              // Don't block the appointment update if WhatsApp fails
              toast.warning(
                'Appointment updated successfully, but WhatsApp notification failed to send'
              );
            }
          } else {
            console.log(
              '[WhatsApp] Skipping update notification - invalid or missing phone number'
            );
          }
        }
  ...
  const handleAddBreak = async (
    ...
    try {
      ...
      toast.error(
        `Failed to add break: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
  };
  ...
  const handleDeleteBreak = async (stylistId: string, breakId: string) => {
    try {
      ...
      if (!breakExists) {
        console.error('Break not found in stylist breaks array:', {
          breakId,
          stylistBreaks: stylist.breaks,
        });
      ...
      if (isWhatsAppEnabled() && editingAppointment) {
        ...
        if (client && client.phone && isValidPhoneNumber(client.phone)) {
          try {
            ...
            const stylist = stylists.find(
              s => s.id === editingAppointment.stylist_id
            );
            ...
            if (service && stylist) {
              ...
              console.log(
                '[WhatsApp] Appointment cancellation notification sent successfully'
              );
            }
          } catch (whatsappError) {
            console.error(
    ...
    try {
      ...
      const clientAppointments = allAppointments.filter(
        (appointment: MergedAppointment) =>
          appointment.client_id === selectedClient.id
      );

      // Sort by date (newest first)
      clientAppointments.sort(
        (a: MergedAppointment, b: MergedAppointment) =>
  ...
  const handleAddNewClient = async (
    clientEntry: ExtendedClientAppointmentEntry
  ) => {
  ...
  const handleAddNewServiceClick = () => {
    ...
    const lastService = entry?.services[entry.services.length - 1];
    const fromTime =
      lastService?.toTime || appointmentTime.startTime || '10:00';
    const defaultStylistId =
      lastService?.stylistId || entry?.stylists[0]?.id || '';
  ...
  const handleInlineServiceAdd = () => {
    ...
    if (inlineServiceData.additionalExperts) {
      const hasIncompleteExperts = inlineServiceData.additionalExperts.some(
        expert => !expert.id || expert.id.trim() === ''
      );
    ...
    const selectedService = activeServices.find(
      s => s.id === inlineServiceData.serviceId
    );
    if (!selectedService) {
    ...
    const selectedStylist = stylists.find(
      s => s.id === inlineServiceData.stylistId
    );
    if (!selectedStylist) {
    ...
    const allExperts = [{ id: selectedStylist.id, name: selectedStylist.name }];

    // Add additional experts if they exist
    if (inlineServiceData.additionalExperts) {
      const validAdditionalExperts = inlineServiceData.additionalExperts.filter(
        expert => expert.id && expert.id.trim() !== ''
      );
    ...
    >
      ...
      >
        ...
        >
          {viewMode === 'calendar' ? (
            <StylistDayView
              stylists={
                (stylists?.map(s => ({ ...s, breaks: s.breaks || [] })) ||
                  []) as any
              }
              appointments={allAppointments}
              services={services}
      ...
      >
        ...
        >
          ...
          <Typography
            variant='h6'
            sx={{ color: '#6B8E23', flex: 1, textAlign: 'center' }}
          >
        ...
        <Box sx={{ p: 3, overflowY: 'auto', flexGrow: 1, bgcolor: '#f5f5f0' }}>
          ...
          >
            ...
            <Autocomplete<Client & { inputValue?: string }, false, false, true>
              ...
              filterOptions={(options, params) => {
                ...
                const isExisting = options.some(
                  option => inputValue === option.full_name
                );
              ...
              renderOption={(props, option) => {
                ...
                if (option.inputValue) {
                  return (
                    <li
                      {...props}
                      style={{ fontStyle: 'italic', color: '#6B8E23' }}
                    >
                      {option.inputValue}
                    </li>
                  );
            ...
            {clientEntries[0]?.client && !clientEntries[0]?.client.id && (
              <TextField
                ...
                error={
                  ...
                  !!(
                    clientEntries[0]?.client?.phone &&
                    !isValidPhoneNumber(clientEntries[0]?.client?.phone)
                  )
                }
                helperText={
                  !clientEntries[0]?.client?.phone
            ...
            {clientEntries[0]?.client && !clientEntries[0]?.client.id && (
              <TextField
                ...
                error={
                  !!(
                    clientEntries[0]?.client?.email &&
                    !isValidEmail(clientEntries[0]?.client?.email)
                  )
                }
                helperText={
                  clientEntries[0]?.client?.email &&
                ...
                <FormControlLabel
                  control={
                    <Checkbox
                      ...
                      onChange={e =>
                        updateEntry(clientEntries[0].id, {
                          isForSomeoneElse: e.target.checked,
                        })
                      }
                      color='primary'
                    />
                ...
                {clientEntries[0].isForSomeoneElse && (
                  ...
                  >
                    ...
                    <TextField
                      ...
                      helperText={
                        !clientEntries[0].bookerName
                          ? "Booker's name is required"
                          : ''
                      }
                      sx={{ mb: 2 }}
                      InputProps={{
                    ...
                    <TextField
                      ...
                      error={
                        ...
                        !!(
                          clientEntries[0].bookerPhone &&
                          !isValidPhoneNumber(clientEntries[0].bookerPhone)
                        )
                      }
                      helperText={
                        !clientEntries[0].bookerPhone
                    ...
                    <TextField
                      ...
                      error={
                        !!(
                          clientEntries[0].bookerEmail &&
                          !isValidEmail(clientEntries[0].bookerEmail)
                        )
                      }
                      helperText={
                        clientEntries[0].bookerEmail &&
          ...
          >
            ...
            <Autocomplete<ServiceOption, false, false, true>
              ...
              options={activeServices.filter(
                service =>
                  (!serviceSearchTerm ||
                    service.name
                      .toLowerCase()
                      .includes(serviceSearchTerm.toLowerCase())) &&
                  (serviceGenderFilter === null ||
                    (service as ServiceWithGender).gender ===
                      serviceGenderFilter)
              )}
              getOptionLabel={option =>
                    ...
                    <Box>
                      ...
                      {(option as ServiceWithGender).gender && (
                        <Typography variant='caption' color='text.secondary'>
                          {(option as ServiceWithGender).gender === 'male'
                            ? 'Male'
                            : 'Female'}
                        </Typography>
                      )}
              ...
              onChange={(_, newValue) => {
                ...
                if (
                  ...
                  setServiceSelectedMessage(
                    `${(newValue as BaseService).name} selected!`
                  );
                  setTimeout(() => setServiceSelectedMessage(''), 2000);
                }
              }}
              ...
              >
                ...
                >
                  ...
                  <IconButton
                    ...
                    color='default'
                    onClick={() =>
                      removeServiceFromEntry(clientEntries[0].id, serviceIndex)
                    }
                  >
                ...
                <Box sx={{ p: 2 }}>
                          ...
                          onDelete={() => {
                            if (clientEntries[0]?.services) {
                              const updatedServices = [
                                ...clientEntries[0].services,
                              ];
                              if (
                                updatedServices[serviceIndex].experts &&
                                updatedServices[serviceIndex].experts!.length >
                                  1
                              ) {
                                // Remove this expert from the experts array
                                updatedServices[serviceIndex].experts!.splice(
                                  expertIndex,
                                  1
                                );
                                ...
                                if (
                                  ...
                                  const newPrimary =
                                    updatedServices[serviceIndex].experts![0];
                  ...
                  <Button
                    ...
                    startIcon={<AddIcon />}
                    onClick={() =>
                      handleAddExpert(clientEntries[0].id, serviceIndex)
                    }
                    sx={{
                      mb: 2,
                      borderColor: '#6B8E23',
                            ...
                            <Autocomplete<Stylist, false, false, false>
                              ...
                              options={stylists
                                .filter(
                                  (stylist: Stylist) =>
                                    ...
                                    !service.experts?.some(
                                      assignedExpert =>
                                        assignedExpert.id === stylist.id
                                    )
                                )
                              ...
                              onChange={(_, newValue: Stylist | null) => {
                                if (newValue && clientEntries[0]?.services) {
                                  ...
                                  if (!updatedServices[serviceIndex].experts)
                                    updatedServices[serviceIndex].experts = [];
                                  updatedServices[serviceIndex].experts![
                                    expertIndex
              ...
              >
                ...
                <Autocomplete
                  ...
                  onChange={(_, newValue) => {
                    ...
                    const [fromHour, fromMinute] = inlineServiceData.fromTime
                      .split(':')
                      .map(Number);

                    let totalMinutes =
                      fromHour * 60 + fromMinute + serviceDuration;
                    const toHour = Math.floor(totalMinutes / 60);
                    const toMinute = totalMinutes % 60;
                ...
                >
                  <Autocomplete<Stylist, false, false, false>
                    fullWidth
                    options={stylists
                      .filter((stylist: Stylist) => stylist.available !== false)
                      .sort((a, b) => a.name.localeCompare(b.name))}
                    getOptionLabel={(option: Stylist) => option.name}
                    value={
                      stylists.find(
                        (s: Stylist) => s.id === inlineServiceData.stylistId
                      ) || null
                    }
                    onChange={(_, newValue: Stylist | null) => {
                      // Added type for newValue
                      setInlineServiceData(prev => ({
                        ...prev,
                ...
                {inlineServiceData.additionalExperts?.map(
                  (expert, expertIndex) => (
                    <Box
                      key={expertIndex}
                      sx={{
                    ...
                    >
                      <Autocomplete<Stylist, false, false, false>
                        ...
                        options={stylists
                          .filter(
                            (stylist: Stylist) => stylist.available !== false
                          )
                          .sort((a, b) => a.name.localeCompare(b.name))}
                        getOptionLabel={(option: Stylist) => option.name}
                        value={
                          stylists.find((s: Stylist) => s.id === expert.id) ||
                          null
                        }
                        onChange={(_, newValue: Stylist | null) => {
                          if (newValue) {
                      ...
                      <FormControl fullWidth size='small'>
                        ...
                        <Select
                          ...
                          onChange={e => {
                            ...
                            if (service) {
                              const [fromHour, fromMinute] = newFromTime
                                .split(':')
                                .map(Number);
                              const serviceDuration = service.duration || 60;
        ...
        >
          ...
          {editingAppointment && (
            <Button
              onClick={() => {
                console.log(
                  '[Create Bill] Clicked. Editing Appointment:',
                  editingAppointment
                );
                console.log(
                  '[Create Bill] Client from clientEntries[0]:',
                  clientEntries[0]?.client
                );
                console.log(
                  '[Create Bill] Services from clientEntries[0]:',
                  clientEntries[0]?.services
                );

                if (!editingAppointment || !clientEntries[0]?.client) {
                  toast.error(
                    'Cannot create bill: Missing appointment or client details.'
                  );
                  console.error(
                    '[Create Bill] Pre-condition failed: editingAppointment or clientEntries[0].client is missing.'
                  );
                  return;
                }
                ...
                if (!clientEntries[0]?.services?.length) {
                  toast.error(
                    'No services found for this appointment to create a bill.'
                  );
                ...
                const servicesForPOS = clientEntries[0].services.map(s => ({
                  ...
                  quantity: 1,
                  experts: s.experts || [
                    { id: s.stylistId, name: s.stylistName },
                  ], // Include all experts
                }));
                ...
                console.log(
                  '[Create Bill] Mapped servicesForPOS:',
                  JSON.stringify(servicesForPOS, null, 2)
                );
                console.log(
                  '[Create Bill] All unique experts:',
                  JSON.stringify(allExperts, null, 2)
                );
                ...
                console.log(
                  '[Create Bill] Final appointmentDataForPOS:',
                  JSON.stringify(appointmentDataForPOS, null, 2)
                );
                navigate('/pos', {
                  state: { appointmentData: appointmentDataForPOS },
                });
                handleCloseDrawer();
              }}
              variant='contained'
      ...
      >
        <DialogTitle id='delete-appointment-title' sx={{ color: '#6B8E23' }}>
          Delete Appointment?
        </DialogTitle>
        <DialogContent>
